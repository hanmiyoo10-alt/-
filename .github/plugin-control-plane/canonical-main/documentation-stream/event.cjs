...
function changedPathsFromPush(payload) {
  ...
  for (const commit of payload.commits || []) {
    for (const key of ['added', 'modified', 'removed']) for (const p of commit[key] || []) paths.add(p);
  }
  return [...paths].sort();
}
...
function classifyPushPaths(paths) {
  ...
  return projectChange ? 'PROJECT' : null;
}
...
function normalizeEvent({ eventName, payload, repository }) {
  ...
  if (eventName === 'issues') {
    ...
    if (labels.has('control-plane:incident')) {
      if (!['opened', 'reopened', 'closed'].includes(action)) return null;
      const eventClass = action === 'closed' ? 'RECOVERY' : 'INCIDENT';
      const event = { schemaVersion: 1, eventClass, sourceType: 'issue', sourceId: String(issue.number), transition: action.toUpperCase(), sha: null, timestamp: issue.updated_at || new Date().toISOString(), summary: compactSummary(issue.title), sourceUrl: issue.html_url || null, evidence: [`issue:#${issue.number}`, `action:${action}`], stable: true };
      event.eventId = eventId(event); return event;
    }
    if (action === 'closed' && /^\[design\]/i.test(issue.title || '')) {
...
module.exports = { EVENT_CLASSES, GENERATED_MARKER, stableJson, eventId, changedPathsFromPush, isDocumentationGenerated, classifyPushPaths, normalizeEvent, markerForEvent, parseEventMarker, renderLiveComment };
