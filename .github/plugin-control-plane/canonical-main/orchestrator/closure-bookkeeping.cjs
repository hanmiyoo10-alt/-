...
function parseQueue(body) {
  ...
  return {valid:Boolean(state), state, activePacket:active ? Number(active) : null, latestPacket:latest ? Number(latest) : null, design:design ? Number(design) : null, activeNone:/Active writable packet:\s*\*\*NONE\*\*/.test(part)};
}
...
function renderQueueIdle(body, {packetNumber,designNumber,packetIdValue,bundle,proofRunId}) {
  ...
  let next=blockReplace(body,QUEUE_START,QUEUE_END,block).replace(/\*\*Queue state:\s*(?:ACTIVE|IDLE)\s*\/\s*CANONICAL-MAIN-V1\.2\*\*/,'**Queue state: IDLE / CANONICAL-MAIN-V1.2**').replace(/- active packet:\s*#[1-9]\d*/i,'- active packet: none').replace(/- latest completed packet:\s*(?:#[1-9]\d*|none)/i,`- latest completed packet: #${packetNumber}`); return appendCompleted(next,packetIdValue);
}
...
function run() {
  ...
  if(queue.valid&&queue.state==='IDLE'&&queue.activeNone&&queue.latestPacket!==id.packet){ note('AUTO_CLOSURE=NO_ACTIVE_PACKET'); return; }
  ...
  const packetDone=eligible.packetState==='DONE'&&packet.state==='closed'&&String(packet.body||'').includes(EVIDENCE_START), q=parseQueue(freshQueue.body), queueDone=q.state==='IDLE'&&q.latestPacket===id.packet; let packetChanged=false;
