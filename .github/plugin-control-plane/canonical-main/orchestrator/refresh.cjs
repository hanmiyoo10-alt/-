...
function mergeIssueCollections(...collections) {
  const byNumber = new Map();
  for (const collection of collections) {
    for (const issue of collection || []) {
      if (Number.isInteger(issue?.number) && issue.number > 0) byNumber.set(issue.number, issue);
    }
  }
  return [...byNumber.values()].sort((a, b) => a.number - b.number);
}
async function loadRefreshIssues(issueStore) {
  const [openIssues, incidentIssues] = await Promise.all([
    issueStore.listIssues('open'),
    issueStore.listIssues('all', 5, ['control-plane:incident']),
  ]);
  return mergeIssueCollections(openIssues, incidentIssues);
}
...
async function refresh(options = {}) {
  ...
  let allIssues = await loadRefreshIssues(issueStore);
  ...
  if (touched.length) { allIssues = await loadRefreshIssues(issueStore); context.allIssues = allIssues; }
  const repaired = await repairIncidentConsistency(issueStore, allIssues);
  if (repaired.length) { allIssues = await loadRefreshIssues(issueStore); context.allIssues = allIssues; }
...
module.exports = {OPS_VIEW_MARKER, LABEL_DEFS, loadRegistry, mergeIssueCollections, loadRefreshIssues, applyIncidentPlan, collectObservations, collectBaseObservations, adapterEvents, observationCoverageValid, repairIncidentConsistency, configuredOpsIssueNumber, validateOpsIssue, resolveOpsIssue, writeOpsSurface, refresh};
