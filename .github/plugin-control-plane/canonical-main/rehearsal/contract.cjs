...
const {loadPolicy, validateEvent, correlationKey} = require('../contract.cjs');
const {previousIncidentState, buildAlertEnvelope, envelopeMarker} = require('../notification.cjs');
const rehearsalConfig = require('./config.json');
const policy = loadPolicy();
const REHEARSAL_ID = rehearsalConfig.id;
const REASON_CODE = rehearsalConfig.reasonCode;
...
function deliveryEnvelope(event,transition,issue){ const key=correlationKey(event),severity=rehearsalConfig.severity,previousState=previousIncidentState(issue); return{key,severity,previousState,envelope:buildAlertEnvelope({event,severity,transition,correlationKey:key,previousState})}; }
module.exports={policy,rehearsalConfig,REHEARSAL_ID,REASON_CODE,markerForKey,markerForEvent,proofMarker,buildRehearsalEvent,incidentLabels,renderIncidentBody,rehearsalState,deliveryEnvelope};
