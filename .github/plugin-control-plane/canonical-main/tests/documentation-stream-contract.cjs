...
const taxonomy = JSON.parse(fs.readFileSync(path.join(root, '.github/plugin-control-plane/taxonomy.json'), 'utf8'));
...
const catalog = renderProjectCatalog({ registry, taxonomy, root });
assert.ok(catalog.includes('plugin:simcore'));
assert.ok(catalog.includes('## Family Navigation Projection'));
assert.ok(catalog.includes('Taxonomy authority posture: `navigation-only`'));
assert.ok(catalog.includes('Path moves authorized: `false`'));
assert.ok(catalog.includes('Identity collapse authorized: `false`'));
assert.ok(catalog.includes('Release mutation authorized: `false`'));
assert.ok(catalog.includes('| PLUGIN | O | Local | plugin:usage-dashboard<br>plugin:devpass<br>plugin:voyage-token-check |'));
assert.ok(catalog.includes('| PRODUCT | X | Mobile Coder Lab | scope:research-product |'));
...
assert.ok(catalog.includes('| plugin:usage-dashboard | Local Usage Dashboard | production | plugins/usage-dashboard/** | releaseBranch=release-usage-dashboard; manifest=plugins/usage-dashboard/runtime/product-manifest.json; artifact=plugins/usage-dashboard/latest.js; releaseSpecDir=.github/usage-dashboard/releases | docs/USAGE_DASHBOARD_GUIDELINES.md |'), 'compatibility landing must not replace Usage Dashboard production authority row');
assert.ok(catalog.includes('| plugin:devpass | DevPass | declared-update-channel | plugins/risu/local/devpass/** | declaredBy=plugins/risu/local/devpass/README.md; artifact=plugins/devpass/latest.js; ref=main | docs/DEVPASS_GUIDELINES.md |'));
assert.ok(catalog.includes('| plugin:voyage-token-check | Voyage Token Check | design-evidence-validation | plugins/risu/local/voyage/** | evidence=plugins/risu/local/voyage/DESIGN_STATUS.md | docs/VOYAGE_TOKEN_CHECK_GUIDELINES.md |'));
...
assert.ok(catalog.includes('plugins/usage-dashboard/**<br>products/usage-dashboard/**<br>plugins/risu/local/usage-dashboard/**'), 'family navigation must preserve Usage Dashboard production/product roots before the Local compatibility landing');
assert.ok(catalog.includes('plugins/risu/local/devpass/**<br>plugins/devpass/**'), 'family navigation must show canonical DevPass project root before fixed update-channel compatibility root');
assert.ok(catalog.includes('plugins/risu/local/voyage/**<br>voyage-token-check/**'), 'family navigation must show canonical Voyage root before legacy compatibility root');
...
assert.match(promoteSource, /taxonomy\.json/);
assert.match(promoteSource, /renderProjectCatalog\(\{ registry, taxonomy, root \}\)/);
