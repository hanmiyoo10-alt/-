...
const issueStoreSource=fs.readFileSync(path.join(base,'infra/issue-store.cjs'),'utf8');assert.match(issueStoreSource,/const getIssue = \(issueNumber\) => client\.api\(`\/issues\/\$\{issueNumber\}`/);assert.match(issueStoreSource,/listIssues\(state = 'open', maxPages = 5, labels = \[\]\)/,'broad issue enumeration must remain bounded');
