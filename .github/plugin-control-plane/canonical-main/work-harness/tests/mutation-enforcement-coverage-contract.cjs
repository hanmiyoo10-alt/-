...
assert.deepEqual(report.requiredProvenRouteKeys, [
...
assert.equal(byCapability.get('usage-dashboard:USAGE_DASHBOARD_CANDIDATE').enforcementState, 'REQUIRED_PROVEN');
...
const missingRequiredProof = clone(evidence);
missingRequiredProof.rows.find((row) => row.capability === 'USAGE_DASHBOARD_CANDIDATE').proofRefs = [];
assert.throws(
  () => evaluateCoverage({ registry, evidence: missingRequiredProof, readSurface }),
  /COVERAGE_PROOF_MISSING:usage-dashboard::USAGE_DASHBOARD_CANDIDATE::CANDIDATE_STATE::GITHUB_WORKFLOW::\.github\/workflows\/usage-dashboard-prepare-candidate\.yml/,
);
