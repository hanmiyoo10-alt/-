...
def _fallback(args: argparse.Namespace, code: str, message: str) -> dict[str, Any]:
    return {
        ...
        "run": _run(args),
        "scope": {"product": "Termux Background GPT", "profile": "TERMUX_BACKGROUND_GPT"},
        "result": "INFRA_ERROR",
        "counts": {"passed": 0, "total": 0, "failed": 0, "warnings": 0},
        "checks": [],
        "reasonCodes": [code],
        "firstFailure": {
            "phase": "termux_background_gpt_adapter",
            "code": code,
...
def _read(path: str) -> dict[str, Any]:
    ...
    if not source.is_file():
        raise ValueError("Termux Background GPT phase receipt missing")
    if source.stat().st_size > MAX_REPORT_BYTES:
        raise ValueError("Termux Background GPT phase receipt exceeds 32 KiB")
    try:
        raw = json.loads(source.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError(f"Termux Background GPT phase receipt unreadable: {exc}") from exc
    if not isinstance(raw, dict):
        raise ValueError("Termux Background GPT phase receipt root must be object")
    return raw


def _validate_receipt(raw: dict[str, Any]) -> dict[str, str]:
    if raw.get("schemaVersion") != 1 or raw.get("receiptKind") != "CI_PHASE_RECEIPT_V1":
        raise ValueError("Termux Background GPT phase receipt contract mismatch")
    phases = raw.get("phases")
    if not isinstance(phases, list) or len(phases) != len(EXPECTED_PHASES):
        raise ValueError("Termux Background GPT phase receipt length mismatch")
    ...
    for expected, item in zip(EXPECTED_PHASES, phases, strict=True):
        if not isinstance(item, dict) or set(item) != {"name", "result"} or item.get("name") != expected:
            raise ValueError("Termux Background GPT phase receipt phase mismatch")
        result = item.get("result")
        if result not in PHASE_RESULTS:
            raise ValueError("Termux Background GPT phase receipt result invalid")
        observed[expected] = result
    if not isinstance(raw.get("metadata"), dict):
        raise ValueError("Termux Background GPT phase receipt metadata invalid")
    return observed
...
def adapt(
    ...
    *,
    ...
    if len(active) > 1:
        raise ValueError("Termux Background GPT phase receipt has multiple RUNNING phases")

    if not active:
        if any(phases[name] != "PASS" for name in EXPECTED_PHASES):
            raise ValueError("Termux Background GPT incomplete receipt has no active failure phase")
        for phase in EXPECTED_PHASES:
            if outcomes[phase] != "success":
                raise ValueError(f"Termux Background GPT PASS {phase} contradicts step outcome")
        return {
            **base,
            "result": "PASS",
    ...
    if any(phases[name] != "PASS" for name in prior):
        raise ValueError("Termux Background GPT RUNNING phase has incomplete predecessor")
    if any(phases[name] != "NOT_RUN" for name in later):
        raise ValueError("Termux Background GPT RUNNING phase has executed successor")
    if any(outcomes[name] != "success" for name in prior):
        raise ValueError("Termux Background GPT passed predecessor contradicts step outcome")
    if any(outcomes[name] != "skipped" for name in later):
        raise ValueError("Termux Background GPT NOT_RUN successor contradicts step outcome")

    outcome = outcomes[phase]
    if outcome == "failure":
        result = "FAIL"
        check_result = "FAIL"
        code = f"TERMUX_BACKGROUND_GPT_{phase.upper()}_FAILED"
        failed = 1
        message = f"authoritative Termux Background GPT {phase} command failed"
    elif outcome == "cancelled":
        result = "CANCELLED"
        check_result = "CANCELLED"
        code = f"TERMUX_BACKGROUND_GPT_{phase.upper()}_CANCELLED"
        failed = 0
        message = f"authoritative Termux Background GPT {phase} command was cancelled"
    else:
        raise ValueError("Termux Background GPT RUNNING phase must have failure or cancelled outcome")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Adapt Termux Background GPT phase receipt to CI_SUMMARY_V1")
    parser.add_argument("--report", required=True)
    parser.add_argument("--output", required=True)
    parser.add_argument("--workflow", default="Termux Background GPT")
    ...
    try:
        ...
        text = str(exc)
        code = (
            "TERMUX_BACKGROUND_GPT_PHASE_RECEIPT_MISSING"
            if "missing" in text.lower()
            else "TERMUX_BACKGROUND_GPT_SUMMARY_SOURCE_INVALID"
        )
