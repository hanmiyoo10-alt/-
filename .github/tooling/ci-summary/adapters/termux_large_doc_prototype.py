...
PRODUCT = "Termux Large Doc Prototype"
PROFILE = "TERMUX_LARGE_DOC_PROTOTYPE"
PREFIX = "TERMUX_LARGE_DOC_PROTOTYPE"
...
def _fallback(args: argparse.Namespace, code: str, message: str) -> dict[str, Any]:
    return {
        ...
        "run": _run(args),
        "scope": {"product": PRODUCT, "profile": PROFILE},
        "result": "INFRA_ERROR",
        "counts": {"passed": 0, "total": 0, "failed": 0, "warnings": 0},
        "checks": [],
        "reasonCodes": [code],
        "firstFailure": {
            "phase": "termux_large_doc_prototype_adapter",
            "code": code,
...
def _read(path: str) -> dict[str, Any]:
    ...
    if not source.is_file():
        raise ValueError(f"{PRODUCT} phase receipt missing")
    if source.stat().st_size > MAX_REPORT_BYTES:
        raise ValueError(f"{PRODUCT} phase receipt exceeds 32 KiB")
    try:
        raw = json.loads(source.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError(f"{PRODUCT} phase receipt unreadable: {exc}") from exc
    if not isinstance(raw, dict):
        raise ValueError(f"{PRODUCT} phase receipt root must be object")
    return raw


def _validate_receipt(raw: dict[str, Any]) -> dict[str, str]:
    if raw.get("schemaVersion") != 1 or raw.get("receiptKind") != "CI_PHASE_RECEIPT_V1":
        raise ValueError(f"{PRODUCT} phase receipt contract mismatch")
    phases = raw.get("phases")
    if not isinstance(phases, list) or len(phases) != len(EXPECTED_PHASES):
        raise ValueError(f"{PRODUCT} phase receipt length mismatch")
    ...
    for expected, item in zip(EXPECTED_PHASES, phases, strict=True):
        if not isinstance(item, dict) or set(item) != {"name", "result"} or item.get("name") != expected:
            raise ValueError(f"{PRODUCT} phase receipt phase mismatch")
        result = item.get("result")
        if result not in PHASE_RESULTS:
            raise ValueError(f"{PRODUCT} phase receipt result invalid")
        observed[expected] = result
    if not isinstance(raw.get("metadata"), dict):
        raise ValueError(f"{PRODUCT} phase receipt metadata invalid")
    return observed
...
def adapt(
    ...
    *,
    ...
    if len(active) > 1:
        raise ValueError(f"{PRODUCT} phase receipt has multiple RUNNING phases")

    if not active:
        if any(phases[name] != "PASS" for name in EXPECTED_PHASES):
            raise ValueError(f"{PRODUCT} incomplete receipt has no active failure phase")
        for phase in EXPECTED_PHASES:
            if outcomes[phase] != "success":
                raise ValueError(f"{PRODUCT} PASS {phase} contradicts step outcome")
        return {
            **base,
            "result": "PASS",
    ...
    if any(phases[name] != "PASS" for name in prior):
        raise ValueError(f"{PRODUCT} RUNNING phase has incomplete predecessor")
    if any(phases[name] != "NOT_RUN" for name in later):
        raise ValueError(f"{PRODUCT} RUNNING phase has executed successor")
    if any(outcomes[name] != "success" for name in prior):
        raise ValueError(f"{PRODUCT} passed predecessor contradicts step outcome")
    if any(outcomes[name] != "skipped" for name in later):
        raise ValueError(f"{PRODUCT} NOT_RUN successor contradicts step outcome")

    outcome = outcomes[phase]
    if outcome == "failure":
        result = "FAIL"
        check_result = "FAIL"
        code = f"{PREFIX}_{phase.upper()}_FAILED"
        failed = 1
        message = f"authoritative {PRODUCT} {phase} command failed"
    elif outcome == "cancelled":
        result = "CANCELLED"
        check_result = "CANCELLED"
        code = f"{PREFIX}_{phase.upper()}_CANCELLED"
        failed = 0
        message = f"authoritative {PRODUCT} {phase} command was cancelled"
    else:
        raise ValueError(f"{PRODUCT} RUNNING phase must have failure or cancelled outcome")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=f"Adapt {PRODUCT} phase receipt to CI_SUMMARY_V1")
    ...
    try:
        ...
        text = str(exc)
        code = f"{PREFIX}_PHASE_RECEIPT_MISSING" if "missing" in text.lower() else f"{PREFIX}_SUMMARY_SOURCE_INVALID"
