...
SPEC = importlib.util.spec_from_file_location("termux_background_gpt", ROOT / "adapters" / "termux_background_gpt.py")
termux_background_gpt = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(termux_background_gpt)
...
def adapt(raw, syntax: str, unit: str):
    return termux_background_gpt.adapt(
        raw,
        workflow="Termux Background GPT",
        run=RUN,
        source_path=".ci-summary/termux-background-gpt-phases.json",
        syntax_outcome=syntax,
        unit_outcome=unit,
    )


class TermuxBackgroundGPTAdapterTests(unittest.TestCase):
    def test_pass(self):
        ...
        self.assertEqual(summary["scope"]["product"], "Termux Background GPT")
        self.assertEqual(summary["scope"]["profile"], "TERMUX_BACKGROUND_GPT")
        self.assertTrue(summary["complete"])

    def test_syntax_failure(self):
        summary = adapt(receipt("RUNNING", "NOT_RUN"), "failure", "skipped")
        self.assertEqual(summary["result"], "FAIL")
        self.assertEqual(summary["reasonCodes"], ["TERMUX_BACKGROUND_GPT_SYNTAX_CHECK_FAILED"])
    ...
    def test_unit_failure(self):
        ...
        self.assertEqual(summary["result"], "FAIL")
        self.assertEqual(summary["reasonCodes"], ["TERMUX_BACKGROUND_GPT_UNIT_TESTS_FAILED"])
    ...
    def test_syntax_cancelled(self):
        ...
        self.assertEqual(summary["reasonCodes"], ["TERMUX_BACKGROUND_GPT_SYNTAX_CHECK_CANCELLED"])

    def test_unit_cancelled(self):
        summary = adapt(receipt("PASS", "RUNNING"), "success", "cancelled")
        self.assertEqual(summary["result"], "CANCELLED")
        self.assertEqual(summary["counts"]["failed"], 0)
        self.assertEqual(summary["reasonCodes"], ["TERMUX_BACKGROUND_GPT_UNIT_TESTS_CANCELLED"])
    ...
    def test_main_missing_receipt_is_infra_error(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(termux_background_gpt.main(args), 0)
            summary = json.loads(output.read_text(encoding="utf-8"))
            self.assertEqual(summary["result"], "INFRA_ERROR")
            self.assertFalse(summary["complete"])
            self.assertEqual(summary["reasonCodes"], ["TERMUX_BACKGROUND_GPT_PHASE_RECEIPT_MISSING"])
    ...
    def test_main_malformed_receipt_is_infra_error(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(termux_background_gpt.main(args), 0)
            summary = json.loads(output.read_text(encoding="utf-8"))
            self.assertEqual(summary["reasonCodes"], ["TERMUX_BACKGROUND_GPT_SUMMARY_SOURCE_INVALID"])
            self.assertFalse(summary["complete"])
