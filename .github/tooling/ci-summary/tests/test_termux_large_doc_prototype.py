...
SPEC = importlib.util.spec_from_file_location("termux_large_doc_prototype", ROOT / "adapters" / "termux_large_doc_prototype.py")
termux_large_doc_prototype = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(termux_large_doc_prototype)
...
def adapt(raw, syntax: str, unit: str):
    return termux_large_doc_prototype.adapt(
        raw,
        workflow="Termux Large Doc Prototype",
        run=RUN,
        source_path=".ci-summary/termux-large-doc-prototype-phases.json",
        syntax_outcome=syntax,
        unit_outcome=unit,
    )


class TermuxLargeDocPrototypeAdapterTests(unittest.TestCase):
    def test_pass(self):
        ...
        self.assertEqual(summary["scope"]["product"], "Termux Large Doc Prototype")
        self.assertEqual(summary["scope"]["profile"], "TERMUX_LARGE_DOC_PROTOTYPE")
        self.assertTrue(summary["complete"])

    def test_syntax_failure(self):
        summary = adapt(receipt("RUNNING", "NOT_RUN"), "failure", "skipped")
        self.assertEqual(summary["result"], "FAIL")
        self.assertEqual(summary["reasonCodes"], ["TERMUX_LARGE_DOC_PROTOTYPE_SYNTAX_CHECK_FAILED"])
    ...
    def test_unit_failure(self):
        ...
        self.assertEqual(summary["result"], "FAIL")
        self.assertEqual(summary["reasonCodes"], ["TERMUX_LARGE_DOC_PROTOTYPE_UNIT_TESTS_FAILED"])
        self.assertEqual(summary["counts"]["passed"], 1)

    def test_syntax_cancelled(self):
        summary = adapt(receipt("RUNNING", "NOT_RUN"), "cancelled", "skipped")
        self.assertEqual(summary["result"], "CANCELLED")
        self.assertEqual(summary["counts"]["failed"], 0)
        self.assertEqual(summary["reasonCodes"], ["TERMUX_LARGE_DOC_PROTOTYPE_SYNTAX_CHECK_CANCELLED"])

    def test_unit_cancelled(self):
        summary = adapt(receipt("PASS", "RUNNING"), "success", "cancelled")
        self.assertEqual(summary["result"], "CANCELLED")
        self.assertEqual(summary["counts"]["failed"], 0)
        self.assertEqual(summary["reasonCodes"], ["TERMUX_LARGE_DOC_PROTOTYPE_UNIT_TESTS_CANCELLED"])
    ...
    def test_invalid_step_outcome_rejected(self):
        with self.assertRaisesRegex(ValueError, "invalid GitHub step outcome"):
            adapt(receipt("PASS", "RUNNING"), "success", "timed_out")
    ...
    def test_main_missing_receipt_is_infra_error(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(termux_large_doc_prototype.main(args), 0)
            summary = json.loads(output.read_text(encoding="utf-8"))
            self.assertEqual(summary["result"], "INFRA_ERROR")
            self.assertFalse(summary["complete"])
            self.assertEqual(summary["reasonCodes"], ["TERMUX_LARGE_DOC_PROTOTYPE_PHASE_RECEIPT_MISSING"])
    ...
    def test_main_malformed_receipt_is_infra_error(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(termux_large_doc_prototype.main(args), 0)
            summary = json.loads(output.read_text(encoding="utf-8"))
            self.assertEqual(summary["reasonCodes"], ["TERMUX_LARGE_DOC_PROTOTYPE_SUMMARY_SOURCE_INVALID"])
            self.assertFalse(summary["complete"])
