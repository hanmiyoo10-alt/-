...
During this review, a one-line placeholder audit file was accidentally written directly to `main` before the review branch was created. It was immediately removed in the next commit before this policy artifact was prepared. No runtime/product/release files were touched, and the repository tree was restored before the normal branch/PR path resumed.

This note preserves the operational anomaly rather than hiding it. It does not authorize direct-main writes and does not weaken the repository main-write contract.
