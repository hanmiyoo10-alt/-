...
Status: **DESIGN + IMPLEMENTATION APPROVED — maintenance only**

## Problem

E14 ancestry-aware candidate materialization is implemented and live-proven by 5.82, but the repository currently uses one overloaded word, `generation`, for two different layers:

- the durable release transaction/wake generation accepted by `release_generation`;
- the candidate Git-DAG ancestry baseline implemented by E14.

The durable request parser and generation-proof switch intentionally stop at E13, while the E14 design document historically says `Generation: E14`. That wording makes a correct architecture look inconsistent.
...
The field name `release_generation` is retained for backward compatibility, but its repository meaning is now explicitly **durable transaction generation**.
...
1. Rename the parser's internal meaning to `DURABLE_TRANSACTION_GENERATION_RE` while preserving exported `GENERATION_RE` as a compatibility alias.
2. Keep accepted request values E9..E13; E14 must remain rejected by the durable transaction parser.
