...
| Feature ID | User surface | Authoritative source | Exact source fields | Capture owner | Allowed normalization / derivation | UNKNOWN rule | Forbidden inference | Privacy / retention | Extra I/O | Contract impact | Evidence | Readiness |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `V-FUNDING-PROVENANCE` | Recent Requests / DevPass funding badge | `TBD` — must be proven by `NV-FUNDING-AUTH` | `TBD` explicit billing/funding field(s) | `TBD` | none until authority is proven | no explicit funding evidence => UNKNOWN | account scope, cost, service tier, model category, `usedMode` alone⇒plan-vs-PAYG funding | raw billing identifiers/transaction details must not enter request state without dedicated privacy design | `TBD` | likely additive recent-request metadata; not authorized yet | #348 + `NV-FUNDING-AUTH` prerequisite | `INVESTIGATE` |
