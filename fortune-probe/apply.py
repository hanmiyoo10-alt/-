...
data['productName'] = 'Fortune Golf Probe 11'
data['identifier'] = 'io.hanmiyoo.fortunegolf.probe11'
data['version'] = '0.1.11'
...
replacement = """        self.core.run_function(address, args).await.map_err(|error| {
            WieError::FatalError(alloc::format!(
                "Probe 11 native call: target={address:#010x}, args={args:#x?}; {error}"
            ))
        })"""
...
replacement = """            context.call_function(self.fn_callback, &[self.ptr_timer, self.param]).await.map_err(|error| {
                WieError::FatalError(alloc::format!(
                    "Probe 11 timer: timer={:#010x}, callback={:#010x}, param={:#010x}; {error}",
...
# Capture the selection while registers still belong to the failing guest call.
p = root / 'wie-core-arm/src/core.rs'
s = p.read_text()
needle = '                    let pc = inner.engine.reg_read(ArmRegister::PC);'
assert s.count(needle) == 1
s = s.replace(needle, needle + '\n                    let selection = fortune_selection_snapshot(&mut *inner.engine, pc);')
s = s.replace('lr={lr:#010x}; resources=', 'lr={lr:#010x};{selection} resources=')
p.write_text(s + (source / 'selection_snapshot.rs').read_text())
