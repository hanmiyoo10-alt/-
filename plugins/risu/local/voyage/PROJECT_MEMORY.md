...
This document is the durable project memory and operating contract for Voyage Token Check.

Canonical repository: `hanmiyoo10-alt/-`

Canonical project root: `plugins/risu/local/voyage/`

Canonical plugin path: `UNKNOWN — verify from production evidence before the first development change`

Production release branch: `UNKNOWN — read the actual repository and production release state first`
...
> Repository placement note: this root is durable project/control memory. Until the canonical plugin source is VERIFIED, do not treat this directory itself as production plugin source. Follow the repository-wide no-secrets rule.
...
- VERIFIED: canonical repository is `hanmiyoo10-alt/-`.
- VERIFIED: this repository contains the existing `local/` root and products including `simcore` and `pocketrisu-helper-mod`.
- VERIFIED: Voyage Token Check has a dedicated canonical root at `plugins/risu/local/voyage/` for durable project memory.
- UNKNOWN: canonical production plugin path for Voyage Token Check.
- UNKNOWN: production release branch for Voyage Token Check.
- UNKNOWN: current production version and manifest source for Voyage Token Check.
...
Replace `<PLUGIN_PATH>` only after the canonical production plugin path is VERIFIED.
