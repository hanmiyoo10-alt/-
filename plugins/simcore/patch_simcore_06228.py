...
for path in paths:
    ...
    if probe_decl not in text:
        raise SystemExit(f'{path}: probe declaration marker missing')
    ...
    if active_marker not in text:
        raise SystemExit(f'{path}: active prompt injection marker missing')
    text = text.replace(active_marker, active_new, 1)

    inactive_marker = '''    } else {\n      lastCore = { active: false, mode: null, issues: [], diagnostics: [] };\n    }\n    // v0.62: do not call setChatToIndex on the request-critical path.\n'''
    inactive_new = '''    } else {\n      lastRuntimePromptBudget = null;\n      lastCore = { active: false, mode: null, issues: [], diagnostics: [] };\n    }\n    // v0.62: do not call setChatToIndex on the request-critical path.\n'''
    if inactive_marker not in text:
        raise SystemExit(f'{path}: inactive prompt marker missing')
    ...
    if cards_marker not in text:
        raise SystemExit(f'{path}: diagnostics card marker missing')
    text = text.replace(cards_marker, cards_new, 1)

    text = text.replace('SimCore v0.62.27 <button', 'SimCore v0.62.28 <button', 1)
    text = text.replace(
        'v0.62.27 Reference Attention Anchor · character card + currently exposed lore pointer · +2 fixed prompt lines',
