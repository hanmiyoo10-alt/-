...
// Full request topology still fingerprints the request, but exposes a separate compact current-user identity.
...
// Same natural user turn, but a different assembled full request: user signature must stay identical while request fingerprint changes.
const reqRetry = [
  { role: 'system', content: 'stable head' },
  { role: 'system', content: 'host retry wrapper' },
  { role: 'user', content: 'same user turn' },
  { role: 'system', content: 'runtime A' },
];
...
// A genuinely new user turn seeds EMA with the first real distinct-turn interval, not zero*alpha.
const t2 = {
  ...tr,
  baseline: false,
...
// Retry of the second user turn still cannot establish the family.
...
// Third distinct natural turn may establish; EMA uses the distinct-to-distinct gap (not retry gap).
const t3 = {
  ...t2,
...
assert.strictEqual(cp.cadenceEmaMs, 10000, '10s then 10s distinct gaps should remain 10s');
assert.strictEqual(cp.stableFloorChars, 70000);
assert.strictEqual(cp.movingFrontierChars, 76000);

// Candidate state v2 intentionally rejects v0.63.38 state so polluted distinct/EMA state is not adopted.
...
// Baseline display no longer fabricates 0ms or a zero frontier.
...
// Representation diagnostics use existing short fingerprints only; no raw output is retained.
...
// Prompt/provider/mirror contracts remain frozen.
