...
const rootText = [
  'CURRENT ROOT FACTS mark the current event as authoritative.',
  'The prior plan is already complete and must not replace this present state.',
  'The user explicitly supplies the current facts for this event.',
  'ROOT END UNIQUE cedar maple birch willow pine.',
].join(' ');
const sourceStart = 'SOURCE START UNIQUE alpha bravo charlie delta echo foxtrot golf hotel india juliet kilo lima.';
const sourceMiddle = 'SOURCE MIDDLE UNIQUE mango nectar orange papaya quince raspberry strawberry tangerine vanilla walnut xigua yam.';
const sourceEnd = 'SOURCE END UNIQUE amber bronze cobalt denim emerald fuchsia graphite hazel ivory jade khaki lavender.';
const sourceText = [sourceStart, sourceMiddle, sourceEnd].join(' ');
const currentShort = '[커뮤니티] 라이브 반응';
const pending = { active: true, mode: 'C', requestLineageRootIndex: 0, requestLineageSourceKind: 'ROOT' };
const chat = [
...
// DUAL: exact safe root + exact safe assistant source.
{
  const request = [
  ...
  assert.strictEqual(r.mapping.sourceAssistantShape, 'EXACT');
  assert.strictEqual(r.mapping.rootUserAnchorMask, 'SME');
  assert.strictEqual(r.mapping.sourceAssistantAnchorMask, 'SME');
  assert.strictEqual(r.mapping.rootUserLeadingGap, 0);
  assert.strictEqual(r.mapping.rootUserTrailingGap, 0);
  assert.strictEqual(r.mapping.sourceAssistantLeadingGap, 0);
  assert.strictEqual(r.mapping.sourceAssistantTrailingGap, 0);
...
// ROOT_ONLY: current root stays concrete even when transformed source loses its end boundary.
{
  const transformedSource = [
    sourceStart,
    sourceMiddle,
    'HOST APPENDED UNRELATED quartz river signal timber ultraviolet violet winter xenon yellow zenith.',
  ].join(' ');
  const request = [
    { role: 'system', content: 'stable prelude' },
    { role: 'user', content: rootText },
...
// UNFENCED: root safety remains mandatory; no new source-only promotion path.
{
  const request = [
    { role: 'system', content: 'stable prelude' },
...
// v0.63.39 retry identity and cadence EMA remain frozen.
