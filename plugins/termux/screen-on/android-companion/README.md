...
The receiver returns distinct ordered-broadcast result codes for authorization required, overlay permission required, ON, OFF, and STATUS. It also exposes one unauthenticated read-only `DIAGNOSTIC` action because pairing may not yet exist when startup fails. That action returns only `startup_phase=<FIXED_VALUE>` from a bounded vocabulary and never returns pairing codes, capability tokens, raw preferences, stack traces, or exception text. PAIR/ON/OFF/STATUS authentication is unchanged. Physical screen-timeout behavior still requires real-device validation.
