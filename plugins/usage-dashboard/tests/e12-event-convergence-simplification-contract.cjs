...
assert.ok(reconciler.includes('Usage Dashboard E9 Exact-SHA Validation'), 'E26 must add bounded validation-completion wake without changing validation authority');
assert.ok(reconciler.includes("github.event.workflow_run.name == 'Usage Dashboard E9 Exact-SHA Validation'"), 'E26 validation workflow_run must be wake-only, not release authority');
...
assert.ok(reconciler.includes('release_validation_convergence_e26.cjs --classify-file'), 'E26 reducer must re-read durable exact validation evidence through deterministic convergence classification');
assert.ok(reconciler.includes("schedule:\n    - cron: '*/5 * * * *'"), 'E12 must retain anti-loss schedule');
assert.ok(!reconciler.includes('github.event.workflow_run.head_sha'), 'E12/E26 wake payload must not become candidate authority');
assert.ok(!reconciler.includes('github.event.workflow_run.head_branch'), 'E12/E26 wake payload must not become branch authority');
...
console.log(`usage-dashboard E12 event convergence contract: OK · ${release.productVersion} · platform-safe validation wake + E26 bounded completion wake + self-describing guard receipt`);
