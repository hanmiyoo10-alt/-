...
function writeArtifactSet(name, version, marker = name) {
  ...
  const manifest = {
    format: 1, product: 'Local Usage Dashboard', productVersion: version, releaseBranch: 'release-usage-dashboard',
    components: {
      ...
      bridgeManager: {mode: 'sidecar-manager', version: '1.2.6', productVersion: version, sha256: hash(manager), bootstrapSha256: hash(bootstrap)},
    },
...

{
  ...
  assert.equal(result.status, 0, result.stderr || result.stdout); assert.match(result.stdout, /ALLOW:3\.0\.0-alpha\.5\.60/);
}
{
  ...
  const result = runGuard(candidate, main, release, true);
  assert.equal(result.status, 2, result.stderr || result.stdout); assert.match(result.stdout, /STALE_CANDIDATE_MAIN/);
}
{
  ...
  const result = runGuard(candidate, main, release, true);
  assert.equal(result.status, 2, result.stderr || result.stdout); assert.match(result.stdout, /STALE_CANDIDATE_RELEASE/);
}
{
  ...
  const result = runGuard(candidate, main, release, true);
  assert.equal(result.status, 0, result.stderr || result.stdout); assert.match(result.stdout, /NOOP_IDENTICAL/);
}
{
  ...
  const result = runGuard(candidate, main, release, true);
  assert.equal(result.status, 3, result.stderr || result.stdout); assert.match(result.stdout, /SAME_VERSION_ARTIFACT_DIVERGENCE/);
}
{
  ...
  assert.equal(result.status, 4, result.stderr || result.stdout); assert.match(result.stdout, /CANDIDATE_AHEAD_OF_MAIN/);
}
{
  ...
  bad.productVersion = 'banana'; bad.components.plugin.version = 'banana'; bad.components.bridgeManager.productVersion = 'banana';
  fs.writeFileSync(candidate.manifestFile, JSON.stringify(bad, null, 2));
  let result = runGuard(candidate, main, release);
  assert.equal(result.status, 4, result.stderr || result.stdout); assert.match(result.stderr, /FAIL_CLOSED/);
  bad.productVersion = '3.0.0-alpha.5.60'; bad.components.plugin.version = bad.productVersion; bad.components.bridgeManager.productVersion = bad.productVersion; bad.product = 'SimCore';
  fs.writeFileSync(candidate.manifestFile, JSON.stringify(bad, null, 2));
  result = runGuard(candidate, main, release);
  assert.equal(result.status, 4, result.stderr || result.stdout); assert.match(result.stderr, /unexpected product/);
}
...
assert.doesNotMatch(workflow, /PAYLOAD_COMMIT/);
...
console.log('usage-dashboard P22 monotonic release integrity: OK · downgrade blocked, same-version divergence blocked, main writes absent');
