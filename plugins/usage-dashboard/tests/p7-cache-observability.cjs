...
assert.ok(diag.includes('Cache semantics:'));
assert.ok(diag.includes('request HIT rate'));
assert.ok(diag.includes('provider cache Read'));
assert.ok(diag.includes('unknown stays unknown'));
