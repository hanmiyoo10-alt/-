...
SOURCE="$OWNER/mcl-m-landing-branch-repair"
TMP=$(mktemp -d)
...
git -C "$SEED" branch mainphone/work "$c2"
git -C "$SEED" remote add origin "$REMOTE"
git -C "$SEED" push -q -u origin main
git -C "$SEED" push -q origin mainphone/work
...
make_fixture() {
  ...
  git -C "$FIX" switch -q -c mainphone/rdc-prep-milestone-2213 "$c1"
  git -C "$FIX" branch mainphone/work "$c2"
  git -C "$FIX" remote set-url origin "file://$REMOTE"
  sed     -e "s|CANONICAL_ORIGIN='https://github.com/hanmiyoo10-alt/-.git'|CANONICAL_ORIGIN='file://$REMOTE'|"     -e "s|REPO='/data/data/com.termux/files/home/nyang-worktrees/mainphone-work'|REPO='$FIX'|"     "$SOURCE" >"$TOOL"
  chmod +x "$TOOL"
}
run_tool() {
  set +e
...
run_tool inspect "$c3"
assert_eq "$RC" 0
assert_eq "$(field schema)" mcl-m-landing-branch-repair.v1
...
remote_target_before=$(git --git-dir="$REMOTE" rev-parse refs/heads/mainphone/work)
origin_before=$(git -C "$FIX" rev-parse refs/remotes/origin/main)
source_before=$(git -C "$FIX" rev-parse refs/heads/mainphone/rdc-prep-milestone-2213)
run_tool apply "$c3"
assert_eq "$RC" 0
...
assert_eq "$(git -C "$FIX" branch --show-current)" mainphone/work
assert_eq "$(git -C "$FIX" rev-parse HEAD)" "$c3"
assert_eq "$(git -C "$FIX" rev-parse refs/heads/mainphone/work)" "$c3"
assert_eq "$(git -C "$FIX" rev-parse refs/heads/mainphone/rdc-prep-milestone-2213)" "$source_before"
assert_eq "$(git --git-dir="$REMOTE" rev-parse refs/heads/mainphone/work)" "$remote_target_before"
...
target_before=$(git -C "$FIX" rev-parse refs/heads/mainphone/work)
run_tool inspect "$c3"
assert_eq "$RC" 3
assert_eq "$(field worktree)" dirty
assert_eq "$(git -C "$FIX" branch --show-current)" "$branch_before"
assert_eq "$(git -C "$FIX" rev-parse refs/heads/mainphone/work)" "$target_before"
pass
...
git -C "$FIX" switch -q mainphone/rdc-prep-milestone-2213
...
git -C "$FIX" branch -f mainphone/rdc-prep-milestone-2213 "$diverged"
git -C "$FIX" switch -q mainphone/rdc-prep-milestone-2213
run_tool inspect "$c3"
assert_eq "$RC" 3
pass
...
git -C "$FIX" switch -q mainphone/rdc-prep-milestone-2213
git -C "$FIX" branch -f mainphone/work "$target_diverged"
run_tool inspect "$c3"
assert_eq "$RC" 3
pass

make_fixture checked-elsewhere
git -C "$FIX" worktree add -q "$TMP/other-worktree" mainphone/work
run_tool inspect "$c3"
assert_eq "$RC" 3
...
git -C "$SEED" push -q -f origin "$remote_diverged":refs/heads/mainphone/work
git -C "$FIX" fetch -q origin refs/heads/mainphone/work:refs/remotes/origin/fixture-target
run_tool inspect "$c3"
assert_eq "$RC" 3
git --git-dir="$REMOTE" update-ref refs/heads/mainphone/work "$c2"
...
grep -Fq "TARGET_BRANCH='mainphone/work'" "$SOURCE" || fail 'fixed target branch missing'
grep -Fq "SOURCE_BRANCH='mainphone/rdc-prep-milestone-2213'" "$SOURCE" || fail 'fixed source branch missing'
grep -Fq "REPO='/data/data/com.termux/files/home/nyang-worktrees/mainphone-work'" "$SOURCE" || fail 'fixed worktree missing'
pass
