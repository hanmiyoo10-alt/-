...
DEST_SERVICE_RUN="$PREFIX/var/service/mcl-m-rdc-supervisor-guard/run"
...
emit_check() {
    ...
    printf '%s\n' \
        'schema=mcl-m-rdc-supervisor-install.v2' \
        "guard=$guard_state" \
...
apply_files() {
    ...
    if [ ! -e "$service_dir" ]; then
        stage="$HOME_DIR/.local/state/mcl-m-rdc-supervisor-guard/install-service.$$"
        rm -rf "$stage"
