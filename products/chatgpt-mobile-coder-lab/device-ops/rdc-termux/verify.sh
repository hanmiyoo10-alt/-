...
RUNTIME_ENV_SHIM="$INSTALL_DIR/runtime-env-forward-shim.cjs"
...
[ -f "$RUNTIME_ENV_SHIM" ] || fail "runtime-env shim missing"
...
grep -Fq '// mcl-rdc-termux-runtime-env-forward:v1' "$RUNTIME_ENV_SHIM" || fail "runtime-env shim ownership marker"
...
grep -Fq "RUNTIME_ENV_SHIM='$RUNTIME_ENV_SHIM'" "$SERVICE_DIR/run" || fail "runtime-env shim path"
grep -Fq -- '--require "$SHIM"' "$SERVICE_DIR/run" || fail "device-name shim preload"
grep -Fq -- '--require "$RUNTIME_ENV_SHIM"' "$SERVICE_DIR/run" || fail "runtime-env shim preload"
! grep -Fq 'NODE_OPTIONS' "$SERVICE_DIR/run" || fail "service uses global Node preload surface"
