...
- base PR #2060 merged: `d9e93115f943138ad7c675fcc675e4a0460714b9`.
- guard-owner durability follow-up #2786 / PR #2878 merged: `0a25b7691bf5d768aced94403b32ebdb50f12cc3`.
- exact merged guard + independent anchor launcher를 M에 backup-first로 배포했고 source hash identity를 확인했다.
- controlled guard hard loss → anchor recovery: PASS.
- controlled anchor hard loss → guard recovery: PASS.
- final guard/anchor cardinality: 1 / 1.
- target tunnel supervision과 localhost health를 유지했고, broad service-daemon/runsvdir restart·network reset·reboot·server mutation은 사용하지 않았다.
- shared top-level `runsvdir`는 여전히 absent이며 사망 원인은 계속 `UNKNOWN`.
- 서버폰 코드/서비스, notification relay, passive reconnect watcher 의미는 변경하지 않았다.

다음 한 단계:
- 추가 배포 대신 자연 운용을 관찰한다. shared `runsvdir` 원인과 whole-Termux 복구는 별도 owner에서 추적한다.
