...
import { runSuite as runV06901Suite } from './host-local-telemetry-v06901.test.mjs';
...
export async function runSuite(ctx) {
  ...
  if (metadata !== '0.69.2') return runV06901Suite(ctx);
  ...
  equal(metadata, '0.69.2', 'metadata release identity');
  ...
  assert(ctx.source.includes('SimCore.define("state-reconcile"'), 'v0.69.2 State Reconcile owner missing');
  ...
  ]) assert(ctx.source.includes(marker), `v0.69.2 frozen owner marker missing: ${marker}`);
  ...
  const accepted = await exact.claimHostLocalOnce(hostWith(capsule('0.69.2')), locationKey, now + 1);
  equal(accepted.status, 'CONSUMED', 'v0.69.2 exact Host-local capsule must be compatible');

  const previous = new BundleLoader(ctx.source).load('runtime-telemetry');
  const rejected = await previous.claimHostLocalOnce(hostWith(capsule('0.69.1')), locationKey, now + 1);
  equal(rejected.status, 'INCOMPATIBLE', 'v0.69.1 capsule must not masquerade as v0.69.2');

  return {
    coverage: 'EXECUTABLE',
    status: 'PASS',
    assertions: [
      { id: 'v06902-metadata-runtime-host-identity-equal', status: 'PASS' },
      { id: 'v06902-recovery-retirement-preserved', status: 'PASS' },
      { id: 'v06902-state-reconcile-kernel-inversion-present', status: 'PASS' },
      { id: 'v06902-direct-owner-wiring-preserved', status: 'PASS' },
      { id: 'host-local-v06902-exact-version-compatible', status: 'PASS' },
      { id: 'host-local-v06901-cross-version-rejected', status: 'PASS' },
    ],
  };
}
