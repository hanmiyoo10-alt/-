...
def load_immutable_builder_source():
    ...
    if result.returncode != 0:
        raise SystemExit('06410_RECOVERY03_SOURCE_UNAVAILABLE')
    ...
    for marker in [
        ...
        if marker not in source:
            raise SystemExit(f'06410_RECOVERY03_SOURCE_INVALID marker={marker}')
    return source
...
def replace_once(text, old, new, label):
    ...
    if count != 1:
        raise SystemExit(f'06410_RECOVERY03_PATCH_ANCHOR_INVALID {label} count={count}')
...
def main():
    ...
    if scope['RELEASE_NOTE'].count('getLocalPluginStorage') != 0:
        raise SystemExit('06410_RECOVERY03_RELEASE_NOTE_NORMALIZATION_FAILED')
    ...
    def release_patch(text):
        ...
        if updated.count("serialization: write?.serialization || 'UNKNOWN'") != 1:
            raise SystemExit('06410_RECOVERY03_PATCH_POSTCONDITION_INVALID serialization-probe')
        if updated.count('serialization ${lastTelemetryCheckpointProbe.serialization}') != 1:
            raise SystemExit('06410_RECOVERY03_PATCH_POSTCONDITION_INVALID serialization-diagnostic')
        return updated
