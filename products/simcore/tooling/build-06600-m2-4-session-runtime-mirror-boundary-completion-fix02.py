...
def run(args, *, text=True, check=True):
    ...
    if check and result.returncode != 0:
        ...
        fail("06600_FIX02_COMMAND_FAILED", f"cmd={' '.join(args)} stderr={stderr.strip()} stdout={stdout.strip()}")
    return result
...
def ensure_failed_candidate_object():
    ...
    if fetched.returncode != 0:
        fail("06600_FIX02_FAILED_CANDIDATE_FETCH_FAILED", fetched.stderr.strip())
    observed = git_text("rev-parse", "FETCH_HEAD")
    if observed != FAILED_CANDIDATE:
        fail("06600_FIX02_FAILED_CANDIDATE_REF_MOVED", f"observed={observed}")
...
def verify_failed_candidate_identity():
    ...
    if parents != [FAILED_CANDIDATE, EXPECTED_PARENT]:
        fail("06600_FIX02_FAILED_CANDIDATE_PARENT_INVALID", " ".join(parents))

    blobs = [git_text("rev-parse", f"{FAILED_CANDIDATE}:{path.as_posix()}") for path in FILES]
    if blobs != [EXPECTED_GIT_BLOB, EXPECTED_GIT_BLOB]:
        fail("06600_FIX02_FAILED_CANDIDATE_BLOB_INVALID", repr(blobs))

    latest = failed_candidate_bytes(FILES[0])
    install = failed_candidate_bytes(FILES[1])
    if latest != install:
        fail("06600_FIX02_FAILED_CANDIDATE_LATEST_INSTALL_DIVERGED")
    if len(latest) != EXPECTED_RAW_BYTES:
        fail("06600_FIX02_FAILED_CANDIDATE_BYTES_INVALID", f"observed={len(latest)}")
    digest = hashlib.sha256(latest).hexdigest()
    if digest != EXPECTED_RAW_SHA256:
        fail("06600_FIX02_FAILED_CANDIDATE_SHA256_INVALID", f"observed={digest}")
    return latest
...
def module_bounds(text, name):
    ...
    if start < 0:
        fail("06600_FIX02_MODULE_MISSING", name)
    next_start = text.find('\nSimCore.define("', start + len(start_token))
    if next_start < 0:
        fail("06600_FIX02_MODULE_END_MISSING", name)
...
def repair_session_export(raw_bytes):
    try:
        ...
        fail("06600_FIX02_RUNTIME_UTF8_INVALID", str(exc))
    ...
    if "require('./recovery')" in session or 'require("./recovery")' in session:
        fail("06600_FIX02_SESSION_RECOVERY_REQUIRE_PRESENT")
    if "recovery." in session:
        fail("06600_FIX02_SESSION_RECOVERY_CALL_PRESENT")

    export_start = session.rfind("module.exports = {")
    if export_start < 0:
        fail("06600_FIX02_SESSION_EXPORT_MISSING")
    export_tail = session[export_start:]
    stale_lines = re.findall(r"(?m)^[ \t]+recovery,\r?$", export_tail)
    if len(stale_lines) != 1:
        fail("06600_FIX02_STALE_RECOVERY_EXPORT_COUNT_INVALID", f"count={len(stale_lines)}")

    repaired_tail, count = re.subn(r"(?m)^[ \t]+recovery,\r?\n", "", export_tail, count=1)
    if count != 1:
        fail("06600_FIX02_STALE_RECOVERY_EXPORT_REMOVE_FAILED")
    repaired_session = session[:export_start] + repaired_tail
    if re.search(r"(?m)^[ \t]+recovery,\r?$", repaired_session):
        fail("06600_FIX02_STALE_RECOVERY_EXPORT_REMAINS")
...
def validate_generated_runtime():
    ...
    if latest != install:
        fail("06600_FIX02_LATEST_INSTALL_DIVERGED")

    for path in FILES:
        syntax = run(["node", "--check", path.as_posix()], check=False)
        if syntax.returncode != 0:
            fail("06600_FIX02_NODE_SYNTAX_FAIL", f"path={path} stderr={syntax.stderr.strip()}")

    if not LEGACY_ADAPTER.is_file():
        fail("06600_FIX02_LEGACY_ADAPTER_MISSING", LEGACY_ADAPTER.as_posix())
    for path in FILES:
        legacy = run(["node", LEGACY_ADAPTER.as_posix(), path.as_posix()], check=False)
        if legacy.returncode != 0:
            fail(
                "06600_FIX02_LEGACY_ADAPTER_FAIL",
                f"path={path} stderr={legacy.stderr.strip()} stdout={legacy.stdout.strip()}",
            )
...
def main():
    for path in FILES:
        if not path.is_file():
            fail("06600_FIX02_PRODUCTION_RUNTIME_MISSING", path.as_posix())
    ...
    if delta <= 0 or delta > 32:
        fail("06600_FIX02_MUTATION_BUDGET_INVALID", f"byte_delta={delta}")
