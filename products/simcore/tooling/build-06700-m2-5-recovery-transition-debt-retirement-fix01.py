...
def assert_candidate(original, updated):
    ...
    # Every surviving physical module remains byte-identical except the three
    # explicitly authorized metadata/guidance surfaces below.
    for name in after_names:
        if name in ("contracts", "runtime-telemetry", "runtime-probe"):
            continue
    ...
    expected_probe = module_text(patch_operator_card(original), "runtime-probe")
    if module_text(updated, "runtime-probe") != expected_probe:
        raise SystemExit("06700_RUNTIME_PROBE_DELTA_NOT_OPERATOR_CARD_ONLY")
