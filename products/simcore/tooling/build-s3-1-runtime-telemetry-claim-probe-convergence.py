...
def one(text, old, new, label):
    ...
    if count != 1:
        fail("S3_1_PATCH_ANCHOR_INVALID", f"{label} count={count}")
...
def module_bounds(text, name):
    ...
    if len(starts) != 1:
        fail("S3_1_MODULE_BOUNDARY_INVALID", f"{name} count={len(starts)}")
    start = starts[0]
...
console.log('S3_1_FNV_EQ_PASS');
"""
    result = subprocess.run(["node", "-e", script], text=True, capture_output=True)
    if result.returncode != 0 or "S3_1_FNV_EQ_PASS" not in result.stdout:
        fail("S3_1_S1_FNV_EQUIVALENCE_FAIL", (result.stderr or result.stdout).strip())
...
"""
    result = subprocess.run(["node", "-e", script], text=True, capture_output=True)
    if result.returncode != 0 or "S3_1_CLAIM_PROBE_EQ_PASS" not in result.stdout:
        fail("S3_1_CLAIM_PROBE_EQUIVALENCE_FAIL", (result.stderr or result.stdout).strip())
...
def verify_prior_stages(p0, p1, p2, p3, p4):
    ...
    for marker in ("function fnv1a32(text)", "lines.map(fnv1a32)", "currentLines.map(fnv1a32)", ROLLING_PREFIX_1, ROLLING_PREFIX_2):
        if marker not in c1:
            fail("S3_1_S1_FNV_MARKER_MISSING", marker[:80])
    if require_surface(c0) != require_surface(c1):
        fail("S3_1_S1_CACHE_REQUIRES_CHANGED")
    fnv_equivalence()
    ...
    if cb.replace(CACHE_EXPORTS_P3, CACHE_EXPORTS_P4, 1) != ca:
        fail("S3_1_S2_3_RUNTIME_CACHE_DELTA_WIDENED")
...
def verify_s3_1(p4, p5):
    ...
    if "const HOST_COMPAT_VERSION = '0.70.3';" not in t4 or "const HOST_COMPAT_VERSION = '0.70.3';" not in t5:
        fail("S3_1_HOST_COMPAT_IDENTITY_CHANGED")
...
def verify_identity(text):
    ...
    if got != [TARGET_VERSION, TARGET_VERSION, TARGET_VERSION]:
        fail("S3_1_CUMULATIVE_IDENTITY_INVALID", repr(got))


def syntax_check(path):
    result = subprocess.run(["node", "--check", str(path)], text=True, capture_output=True)
    if result.returncode != 0:
        fail("S3_1_NODE_SYNTAX_FAIL", (result.stderr or result.stdout).strip())
...
def main():
    ...
    for path in FILES:
        path.write_text(p5, encoding="utf-8")
        syntax_check(path)
    if FILES[0].read_bytes() != FILES[1].read_bytes():
        fail("S3_1_OUTPUT_LATEST_INSTALL_DIVERGED")
    print("S3_1_BUILD_PASS")
