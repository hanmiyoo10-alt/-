...
def one(text, old, new, label):
    ...
    if count != 1:
        fail("S3_2_PATCH_ANCHOR_INVALID", f"{label} count={count}")
...
def module_bounds(text, name):
    ...
    if len(starts) != 1:
        fail("S3_2_MODULE_BOUNDARY_INVALID", f"{name} count={len(starts)}")
    start = starts[0]
...
def function_slice(source, name):
    ...
    if len(matches) != 1:
        fail("S3_2_FUNCTION_BOUNDARY_INVALID", f"{name} starts={[m.start() for m in matches]}")
...
console.log('S3_2_FNV_EQ_PASS');
"""
    result = subprocess.run(["node", "-e", script], text=True, capture_output=True)
    if result.returncode != 0 or "S3_2_FNV_EQ_PASS" not in result.stdout:
        fail("S3_2_S1_FNV_EQUIVALENCE_FAIL", (result.stderr or result.stdout).strip())
...
console.log('S3_2_CLAIM_PROBE_EQ_PASS');
"""
    result = subprocess.run(["node", "-e", script], text=True, capture_output=True)
    if result.returncode != 0 or "S3_2_CLAIM_PROBE_EQ_PASS" not in result.stdout:
        fail("S3_2_CLAIM_PROBE_EQUIVALENCE_FAIL", (result.stderr or result.stdout).strip())
...
console.log('S3_2_SESSION_CANDIDATE_EQ_PASS');
"""
    result = subprocess.run(["node", "-e", script], text=True, capture_output=True)
    if result.returncode != 0 or "S3_2_SESSION_CANDIDATE_EQ_PASS" not in result.stdout:
        fail("S3_2_SESSION_CANDIDATE_EQUIVALENCE_FAIL", (result.stderr or result.stdout).strip())
...
def verify_prior_stages(p0, p1, p2, p3, p4):
    ...
    if "function compileRuntimePrompt(state)" in p2 or "function renderRuntimePrompt(state)" in p2 or S2_1_SESSION_ALIAS in p2 or S2_1_SESSION_EXPORT in p2:
        fail("S3_2_S2_1_DEAD_SEAM_SURVIVED")
    if "function compileRuntimePromptParts(state)" not in module_text(p2, "prompt") or "const promptCompiled = compileRuntimePromptParts(state);" not in module_text(p2, "session"):
        fail("S3_2_S2_1_LIVE_PATH_MISSING")

    for marker in S2_2_EXPORTS:
        if marker in module_text(p3, "session"):
            fail("S3_2_S2_2_DEAD_EXPORT_SURVIVED", marker.strip())
    for marker in ("function inspectPreviousBEndOutput(historyMessages, sendIndex) {", "structure.validateStructure(prepared.content, base.pending)", "lifecycle.prepareTurn(base, userText, promptProbe, sendIndex, previousOutputFacts)", "CoreRulesetSession,", "fingerprintText: kernel.fingerprintText,"):
        if marker not in module_text(p3, "session"):
            fail("S3_2_S2_2_LIVE_PATH_MISSING", marker)
    ...
    if cb.replace(CACHE_EXPORTS_P3, CACHE_EXPORTS_P4, 1) != ca:
        fail("S3_2_S2_3_RUNTIME_CACHE_DELTA_WIDENED")
    if tb.replace(TOPO_EXPORTS_P3, TOPO_EXPORTS_P4, 1) != ta:
        fail("S3_2_S2_3_RUNTIME_TOPOLOGY_DELTA_WIDENED")
    if require_surface(cb) != require_surface(ca) or require_surface(tb) != require_surface(ta):
        fail("S3_2_S2_3_REQUIRE_SURFACE_CHANGED")
    for name in module_names(p3):
        if name not in ("runtime-cache", "runtime-topology") and module_text(p3, name) != module_text(p4, name):
            fail("S3_2_S2_3_NON_TARGET_MODULE_CHANGED", name)
    same_counts(p0, p1, SIDE_EFFECT_MARKERS, "S3_2_S1_SIDE_EFFECT_CHANGED")
    same_counts(p1, p2, SIDE_EFFECT_MARKERS, "S3_2_S2_1_SIDE_EFFECT_CHANGED")
    same_counts(p2, p3, SIDE_EFFECT_MARKERS, "S3_2_S2_2_SIDE_EFFECT_CHANGED")
    same_counts(p3, p4, SIDE_EFFECT_MARKERS, "S3_2_S2_3_SIDE_EFFECT_CHANGED")


def verify_s3_1(p4, p5):
    if module_names(p4) != module_names(p5):
        fail("S3_2_S3_1_MODULE_GRAPH_CHANGED")
    for name in module_names(p4):
        if name != "runtime-telemetry" and module_text(p4, name) != module_text(p5, name):
            fail("S3_2_S3_1_NON_TARGET_MODULE_CHANGED", name)
    ...
    if expected != t5:
        fail("S3_2_S3_1_RUNTIME_TELEMETRY_DELTA_WIDENED")
    if require_surface(t4) != require_surface(t5):
        fail("S3_2_S3_1_REQUIRE_SURFACE_CHANGED")
    if t5.count("function recordClaimSelection(") != 1 or t5.count("recordClaimSelection(") != 6:
        fail("S3_2_S3_1_HELPER_COUNT_INVALID")
    ...
    if not exports_match or "recordClaimSelection" in exports_match.group(0):
        fail("S3_2_S3_1_HELPER_EXPORT_INVALID")
...
def verify_s3_2(p5, p6):
    ...
    if not exports_match or "sessionCandidateResult" in exports_match.group(0):
        fail("S3_2_HELPER_EXPORT_INVALID")
    ...
    for name in frozen_functions:
        if function_slice(t5, name) != function_slice(t6, name):
            fail("S3_2_FROZEN_FUNCTION_CHANGED", name)
    ...
    if new_take.strip() != expected_take.strip():
        fail("S3_2_TAKE_SESSION_CANDIDATE_SHAPE_INVALID")
    if old_take.strip() != S3_2_OLD.strip():
        fail("S3_2_PARENT_TAKE_SESSION_CANDIDATE_DRIFT")

    for marker in SESSION_SIDE_EFFECT_MARKERS:
        if old_take.count(marker) != 1 or new_take.count(marker) != 1:
            fail("S3_2_SESSION_SIDE_EFFECT_COUNT_CHANGED", marker)
    order_markers = (
        "candidate.storage.getItem(SESSION_KEY)",
        "if (raw == null)",
        "candidate.storage.removeItem(SESSION_KEY)",
    ...
    if any(pos < 0 for pos in old_positions + new_positions):
        fail("S3_2_SESSION_ORDER_MARKER_MISSING")
    if old_positions != sorted(old_positions) or new_positions != sorted(new_positions):
        fail("S3_2_SESSION_SIDE_EFFECT_ORDER_CHANGED", f"old={old_positions} new={new_positions}")

    for status in ("failed", "empty", "oversize", "available", "malformed"):
        if old_take.count(f"status: '{status}'") != 1:
            fail("S3_2_PARENT_STATUS_MAPPING_DRIFT", status)
        if new_take.count(f"'{status}'") != 1:
            fail("S3_2_NEW_STATUS_MAPPING_INVALID", status)
    if "if (!candidate) return null;" not in new_take:
        fail("S3_2_NULL_CANDIDATE_BEHAVIOR_CHANGED")

    for marker in TELEMETRY_CONSTANTS:
        if t5.count(marker) != 1 or t6.count(marker) != 1:
            fail("S3_2_TELEMETRY_CONSTANT_CHANGED", marker)
    if "const HOST_COMPAT_VERSION = '0.70.3';" not in t5 or "const HOST_COMPAT_VERSION = '0.70.3';" not in t6:
        fail("S3_2_HOST_COMPAT_IDENTITY_CHANGED")

    protected = (
        "recordClaimSelection", "claimHostLocalOnce", "getHostLocalTelemetryStoreOnce",
    ...
    same_counts(p5, p6, protected, "S3_2_PROTECTED_MARKER_CHANGED")
    same_counts(p5, p6, SIDE_EFFECT_MARKERS, "S3_2_SIDE_EFFECT_CHANGED")
...
def verify_identity(text):
    ...
    if got != [TARGET_VERSION, TARGET_VERSION, TARGET_VERSION]:
        fail("S3_2_CUMULATIVE_IDENTITY_INVALID", repr(got))


def syntax_check(path):
    result = subprocess.run(["node", "--check", str(path)], text=True, capture_output=True)
    if result.returncode != 0:
        fail("S3_2_NODE_SYNTAX_FAIL", (result.stderr or result.stdout).strip())
...
def main():
    ...
    for path in FILES:
        path.write_text(p6, encoding="utf-8")
        syntax_check(path)
    if FILES[0].read_bytes() != FILES[1].read_bytes():
        fail("S3_2_OUTPUT_LATEST_INSTALL_DIVERGED")
    print("S3_2_BUILD_PASS")
