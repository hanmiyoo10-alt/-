...
def one(text, old, new, label):
    ...
    if count != 1:
        fail("S4_3_PATCH_ANCHOR_INVALID", f"{label} count={count}")
...
def many(text, old, new, expected, label):
    count = text.count(old)
    if count != expected:
        fail("S4_3_PATCH_ANCHOR_INVALID", f"{label} count={count} expected={expected}")
...
def module_bounds(text, name):
    ...
    if len(starts) != 1:
        fail("S4_3_MODULE_BOUNDARY_INVALID", f"{name} count={len(starts)}")
    start = starts[0]
...
def bounded_module_text(text, name, names):
    ...
    if name != "runtime-probe":
        fail("S4_3_LAST_MODULE_IDENTITY_CHANGED", name)
    token = f'SimCore.define("{name}", function (require, module, exports) {{'
    starts = [i for i in range(len(text)) if text.startswith(token, i)]
    if len(starts) != 1:
        fail("S4_3_LAST_MODULE_BOUNDARY_INVALID", f"{name} count={len(starts)}")
    end = text.find("\n\n(async () => {", starts[0] + len(token))
    if end < 0:
        fail("S4_3_OUTER_SHELL_BOUNDARY_MISSING")
...
console.log('S4_3_BASE_EQ_PASS');
"""
    result = subprocess.run(["node", "-e", script], text=True, capture_output=True)
    if result.returncode != 0 or "S4_3_BASE_EQ_PASS" not in result.stdout:
        fail("S4_3_BASE_EQUIVALENCE_FAIL", (result.stderr or result.stdout).strip())
...
for (const chat of chats) {
  const oldFallback = fallback(chat), newFallback = fallback(chat);
  ...
  for (const pending of [{a:false,s:-1},{a:false,s:3},{a:true,s:3},{a:true,s:-1}]) {
    if (resolve(pending.a,pending.s,oldFallback) !== resolve(pending.a,pending.s,newFallback)) throw new Error('RESOLVE_INPUT_DIFF');
  }
}
console.log('S4_3_FALLBACK_EQ_PASS');
"""
    result = subprocess.run(["node", "-e", script], text=True, capture_output=True)
    if result.returncode != 0 or "S4_3_FALLBACK_EQ_PASS" not in result.stdout:
        fail("S4_3_FALLBACK_EQUIVALENCE_FAIL", (result.stderr or result.stdout).strip())


def branch_equivalence_harness():
    script = r"""
function oldShape(pending) {
  const events = []; let ticks = 0; const now = () => { ticks += 1; events.push('time:'+ticks); return ticks; };
  let template=null,lineage=null,community=null;
  if (pending) { events.push('template'); template={mode:pending.mode,at:now()}; } else { events.push('clear-template'); template=null; }
  if (pending) { const l=pending.requestLineage||{}; events.push('lineage'); lineage={rootMode:l.rootMode||null,at:now()}; } else { events.push('clear-lineage'); lineage=null; }
  if (pending) { events.push('community'); community={eligible:!!pending.eligible,at:now()}; } else { events.push('clear-community'); community=null; }
  return {events,template,lineage,community,ticks};
}
function newShape(pending) {
  const events = []; let ticks = 0; const now = () => { ticks += 1; events.push('time:'+ticks); return ticks; };
  let template=null,lineage=null,community=null;
  if (pending) {
    events.push('template'); template={mode:pending.mode,at:now()};
    const l=pending.requestLineage||{}; events.push('lineage'); lineage={rootMode:l.rootMode||null,at:now()};
    events.push('community'); community={eligible:!!pending.eligible,at:now()};
  } else {
    events.push('clear-template'); template=null;
    events.push('clear-lineage'); lineage=null;
    events.push('clear-community'); community=null;
  }
  return {events,template,lineage,community,ticks};
}
for (const pending of [null,{mode:'A',eligible:true,requestLineage:{rootMode:'A'}}]) {
  const a=oldShape(pending), b=newShape(pending);
  if (JSON.stringify(a)!==JSON.stringify(b)) throw new Error('PENDING_BRANCH_DIFF:'+JSON.stringify({a,b}));
}
console.log('S4_3_BRANCH_EQ_PASS');
"""
    result = subprocess.run(["node", "-e", script], text=True, capture_output=True)
    if result.returncode != 0 or "S4_3_BRANCH_EQ_PASS" not in result.stdout:
        fail("S4_3_BRANCH_EQUIVALENCE_FAIL", (result.stderr or result.stdout).strip())
...
def verify_p0_p8(stages):
    ...
    if not names or any(module_names(x) != names for x in stages[1:]):
        fail("S4_3_P8_MODULE_GRAPH_CHANGED")
    if require_surface(p0) != require_surface(p8):
        fail("S4_3_P8_REQUIRE_SURFACE_CHANGED")
    same_counts(p0, p8, SIDE_EFFECT_MARKERS, "S4_3_P8_SIDE_EFFECT_CHANGED")
    same_counts(p0, p8, PROTECTED_MARKERS, "S4_3_P8_PROTECTED_MARKER_CHANGED")
    c0 = module_text(p0, "runtime-cache")
    c1 = module_text(p1, "runtime-cache")
    if c0.count("0x811c9dc5") != 5 or c1.count("0x811c9dc5") != 3:
        fail("S4_3_S1_FNV_SHAPE_INVALID")
    for marker in ("function fnv1a32(text)", "lines.map(fnv1a32)", "currentLines.map(fnv1a32)"):
        if marker not in c1: fail("S4_3_S1_MARKER_MISSING", marker)
    if "function compileRuntimePrompt(state)" in p2 or "function renderRuntimePrompt(state)" in p2 or S2_1_SESSION_ALIAS in p2 or S2_1_SESSION_EXPORT in p2:
        fail("S4_3_S2_1_DEAD_SEAM_SURVIVED")
    for marker in S2_2_EXPORTS:
        if marker in module_text(p3, "session"): fail("S4_3_S2_2_DEAD_EXPORT_SURVIVED", marker.strip())
    if CACHE_EXPORTS_P4 not in module_text(p4, "runtime-cache") or TOPO_EXPORTS_P4 not in module_text(p4, "runtime-topology"):
        fail("S4_3_S2_3_EXPORT_SHAPE_INVALID")
    t5, t6, t7, t8 = [module_text(x, "runtime-telemetry") for x in (p5,p6,p7,p8)]
    if t5.count("function recordClaimSelection(") != 1 or t5.count("recordClaimSelection(") != 6: fail("S4_3_S3_1_HELPER_COUNT_INVALID")
    if t6.count("function sessionCandidateResult(") != 1 or t6.count("sessionCandidateResult(") != 6: fail("S4_3_S3_2_HELPER_COUNT_INVALID")
    if t7.count("function sessionSurfaceResult(") != 1 or t7.count("sessionSurfaceResult(") != 6: fail("S4_3_S3_3_HELPER_COUNT_INVALID")
    if t8.count("function sessionStorageCandidate(") != 1 or t8.count("sessionStorageCandidate(") != 6: fail("S4_3_S3_4_HELPER_COUNT_INVALID")
    if t7.replace(S3_4_OLD, S3_4_NEW, 1) != t8: fail("S4_3_P7_P8_DELTA_WIDENED")
    for marker in TELEMETRY_CONSTANTS:
        if any(module_text(x, "runtime-telemetry").count(marker) != 1 for x in (p4,p5,p6,p7,p8)): fail("S4_3_TELEMETRY_CONSTANT_CHANGED", marker)
    base_equivalence_harness()


def verify_s4_1(p8, p9):
    names = module_names(p8)
    if module_names(p9) != names or require_surface(p8) != require_surface(p9): fail("S4_3_P9_GRAPH_CHANGED")
    same_counts(p8, p9, SIDE_EFFECT_MARKERS, "S4_3_P9_SIDE_EFFECT_CHANGED")
    same_counts(p8, p9, PROTECTED_MARKERS, "S4_3_P9_PROTECTED_MARKER_CHANGED")
    same_counts(p8, p9, S4_CALL_MARKERS, "S4_3_P9_CALL_SURFACE_CHANGED")
    for name in names:
        if bounded_module_text(p8, name, names) != bounded_module_text(p9, name, names): fail("S4_3_P9_MODULE_CHANGED", name)
    expected = p8.replace(S4_1_HELPER_OLD, S4_1_HELPER_NEW, 1)
    expected = expected.replace(S4_1_PREP_OLD, S4_1_PREP_NEW).replace(S4_1_PROCESS_OLD, S4_1_PROCESS_NEW)
    expected = expected.replace(S4_1_BEFORE_OLD, S4_1_BEFORE_NEW).replace(S4_1_OUTPUT_OLD, S4_1_OUTPUT_NEW)
    if expected != p9: fail("S4_3_P8_P9_DELTA_WIDENED")
    if p9.count("function guardCurrentRuntime(") != 1 or p9.count("guardCurrentRuntime(") != 11: fail("S4_3_P9_GUARD_COUNT_INVALID")
    if p9.count("dropStaleRuntime();") != 1 or p9.count("staleRuntimeDrops += 1;") != 1: fail("S4_3_P9_STALE_ACCOUNTING_CHANGED")
    if p9.count(S4_1_POSITIVE_TELEMETRY_GUARD) != 1: fail("S4_3_P9_POSITIVE_GUARD_CHANGED")


def verify_s4_2(p9, p10):
    names = module_names(p9)
    if module_names(p10) != names or require_surface(p9) != require_surface(p10): fail("S4_3_P10_GRAPH_CHANGED")
    same_counts(p9, p10, SIDE_EFFECT_MARKERS, "S4_3_P10_SIDE_EFFECT_CHANGED")
    same_counts(p9, p10, PROTECTED_MARKERS, "S4_3_P10_PROTECTED_MARKER_CHANGED")
    same_counts(p9, p10, S4_CALL_MARKERS, "S4_3_P10_CALL_SURFACE_CHANGED")
    for name in names:
        if bounded_module_text(p9, name, names) != bounded_module_text(p10, name, names): fail("S4_3_P10_MODULE_CHANGED", name)
    expected = p9.replace(S4_2_DEF_OLD, S4_2_DEF_NEW, 1).replace(S4_2_CALL_OLD, S4_2_CALL_NEW, 1)
    if expected != p10: fail("S4_3_P9_P10_DELTA_WIDENED")
    if p10.count(S4_2_FALLBACK_EXPR) != 1 or p10.count(S4_2_SESSION_RESOLVE) != 1 or p10.count(S4_2_RESOLVE_CALL) != 1: fail("S4_3_P10_FALLBACK_INVARIANT_CHANGED")
...
def post_onsend_region(text):
    ...
    if start < 0 or end < 0: fail("S4_3_PENDING_REGION_MISSING")
    return text[start:end]


def verify_s4_3(p10, p11):
    names = module_names(p10)
    if module_names(p11) != names or require_surface(p10) != require_surface(p11): fail("S4_3_P11_GRAPH_CHANGED")
    same_counts(p10, p11, SIDE_EFFECT_MARKERS, "S4_3_P11_SIDE_EFFECT_CHANGED")
    same_counts(p10, p11, PROTECTED_MARKERS, "S4_3_P11_PROTECTED_MARKER_CHANGED")
    same_counts(p10, p11, S4_CALL_MARKERS, "S4_3_P11_CALL_SURFACE_CHANGED")
    for name in names:
        if bounded_module_text(p10, name, names) != bounded_module_text(p11, name, names): fail("S4_3_P11_MODULE_CHANGED", name)
    expected = p10.replace(S4_3_TEMPLATE_BRIDGE_OLD, S4_3_TEMPLATE_BRIDGE_NEW, 1)
    expected = expected.replace(S4_3_LINEAGE_BRIDGE_OLD, S4_3_LINEAGE_BRIDGE_NEW, 1)
    expected = expected.replace(S4_3_FINAL_CLEAR_OLD, S4_3_FINAL_CLEAR_NEW, 1)
    if expected != p11: fail("S4_3_P10_P11_DELTA_WIDENED")
    if p10.count("      if (pendingProbe) {") != 3 or p11.count("      if (pendingProbe) {") != 1: fail("S4_3_PENDING_BRANCH_COUNT_INVALID")
    if p10.count(S4_3_NARRATIVE) != 1 or p11.count(S4_3_NARRATIVE) != 1: fail("S4_3_NARRATIVE_CONDITION_CHANGED")
    for marker in S4_3_PROJECTIONS:
        if p10.count(marker) != p11.count(marker): fail("S4_3_PROJECTION_COUNT_CHANGED", marker)
    r10, r11 = post_onsend_region(p10), post_onsend_region(p11)
    if r10.count("at: Date.now(),") != r11.count("at: Date.now(),"): fail("S4_3_TIMESTAMP_COUNT_CHANGED")
    order = [r11.find(x) for x in S4_3_PROJECTIONS]
    if any(x < 0 for x in order) or order != sorted(order): fail("S4_3_TRUE_PATH_ORDER_CHANGED", repr(order))
    clear_order = [r11.rfind(x) for x in (
        "lastTemplateRecurrenceProbe = null;",
        "lastRequestLineageProbe = null;",
        "lastCommunitySourceHandoffProbe = null;",
    )]
    if any(x < 0 for x in clear_order) or clear_order != sorted(clear_order): fail("S4_3_FALSE_PATH_ORDER_CHANGED", repr(clear_order))
...
def verify_identity(text):
    ...
    if got != [TARGET_VERSION, TARGET_VERSION, TARGET_VERSION]: fail("S4_3_CUMULATIVE_IDENTITY_INVALID", repr(got))


def syntax_check(path):
    result = subprocess.run(["node", "--check", str(path)], text=True, capture_output=True)
    if result.returncode != 0: fail("S4_3_NODE_SYNTAX_FAIL", (result.stderr or result.stdout).strip())


def main():
    originals = []
    for path in FILES:
        if not path.exists(): fail("S4_3_SOURCE_MISSING", str(path))
        originals.append(path.read_text(encoding="utf-8"))
    if originals[0] != originals[1]: fail("S4_3_PARENT_LATEST_INSTALL_DIVERGED")
    if originals[0].count(f"//@version {FROM_VERSION}") != 1: fail("S4_3_PARENT_VERSION_MISMATCH")
    ...
    p7 = apply_s3_3(p6)
    p8 = apply_s3_4(p7)
    verify_p0_p8((p0,p1,p2,p3,p4,p5,p6,p7,p8))
    verify_identity(p8)
    ...
    for path in FILES:
        path.write_text(p11, encoding="utf-8")
        syntax_check(path)
    if FILES[0].read_bytes() != FILES[1].read_bytes(): fail("S4_3_OUTPUT_LATEST_INSTALL_DIVERGED")
    print("S4_3_BUILD_PASS")
