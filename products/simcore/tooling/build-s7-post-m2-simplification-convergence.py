...
S7_OLD_RELEASE_HEADER = "// v0.70.3 Runtime Cache Hash Primitive Convergence:"
S7_NEW_RELEASE_HEADER = "// v0.70.3 Post-M2 Simplification Convergence:"
...
def apply_s7_identity(p12):
    out = one(p12, S7_OLD_RELEASE_HEADER, S7_NEW_RELEASE_HEADER, "s7-release-headline")
    return one(out, S7_OLD_OPERATOR, S7_NEW_OPERATOR, "s7-operator-card")

def verify_s7_identity_delta(p12, final):
    expected = p12.replace(S7_OLD_RELEASE_HEADER, S7_NEW_RELEASE_HEADER, 1).replace(S7_OLD_OPERATOR, S7_NEW_OPERATOR, 1)
    if expected != final: fail("S7_IDENTITY_DELTA_WIDENED")
    if p12.count(S7_OLD_RELEASE_HEADER) != 1 or p12.count(S7_OLD_OPERATOR) != 1: fail("S7_P12_IDENTITY_CARDINALITY_INVALID")
    if final.count(S7_NEW_RELEASE_HEADER) != 1 or final.count(S7_NEW_OPERATOR) != 1: fail("S7_FINAL_IDENTITY_CARDINALITY_INVALID")
    if S7_OLD_RELEASE_HEADER in final or S7_OLD_OPERATOR in final: fail("S7_OLD_IDENTITY_SURVIVED")
...
def main():
    ...
    final=apply_s7_identity(p12); verify_s7_identity_delta(p12,final); verify_identity(final)
