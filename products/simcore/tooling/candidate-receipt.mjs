...
export function deriveReceipt(request, report, verifierCommit){
  ...
  return {
    schemaVersion:1,product:'SimCore',intentId:request.intentId,releaseId,
    candidateDisposition:report.candidateDisposition,
    expectedProductionCommit:report.expectedProductionCommit,
    sourceCommit:report.sourceCommit,
    candidateCommit:report.candidateCommit,
    candidateReleaseBlob:report.candidateReleaseBlob,
    candidateFetchRef:report.candidateFetchRef,
    builderPath:report.builderPath,builderSha256:report.builderSha256,
    verifierCommit,verificationSuite:report.verificationSuite,
    verificationReportSha256:report.verificationReportSha256,
...
export function deriveSpecShadow(request,receipt){
  ...
  const derivedSpec={
    schemaVersion:1,releaseId:request.releaseId,product:'SimCore',version:request.targetVersion,
    releaseName:request.releaseName,releaseMode:request.releaseMode,
    candidateCommit:receipt.candidateCommit,expectedProductionCommit:receipt.expectedProductionCommit,
    candidateReleaseBlob:receipt.candidateReleaseBlob,primaryGoalId:request.primaryGoalId,
    changeClass:request.changeClass,evidenceRefs:request.evidenceRefs,liveGate:request.liveGate
  };
...
function main(){const a=parseArgs(process.argv.slice(2));const req=JSON.parse(fs.readFileSync(a.request,'utf8'));const reportBytes=fs.readFileSync(a['candidate-report']);const report=JSON.parse(reportBytes);if(report.verificationReportSha256&&report.verificationReportSha256!==sha256(fs.readFileSync(report.verificationReportPath||a['candidate-report']))){}const receipt=deriveReceipt(req,report,a['verifier-commit']);const shadow=deriveSpecShadow(req,receipt);writeJson(a.receipt,receipt);writeJson(a['spec-shadow'],shadow);console.log(`SIMCORE_CANDIDATE_RECEIPT_PASS intent=${receipt.intentId} release=${receipt.releaseId}`);}
