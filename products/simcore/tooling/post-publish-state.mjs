...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    const a = argv[i];
    if (!a.startsWith('--')) fail('POST_PUBLISH_ARGS_INVALID', a);
    const key = a.slice(2);
    const value = argv[++i];
    if (value == null || value.startsWith('--')) fail('POST_PUBLISH_ARGS_INVALID', key);
    out[key] = value;
  }
  return out;
}
function under(root, rel) {
  if (!rel || typeof rel !== 'string' || path.isAbsolute(rel)) fail('POST_PUBLISH_PATH_INVALID', String(rel));
  const p = path.resolve(root, rel);
  if (p !== root && !p.startsWith(root + path.sep)) fail('POST_PUBLISH_PATH_OUTSIDE_ROOT', rel);
  return p;
}
...
export function run(argv = process.argv.slice(2)) {
  ...
  for (const k of ['root','input','production-identity','report']) if (!a[k]) fail('POST_PUBLISH_ARGS_INVALID', k);
  ...
  const workDirRel = a['work-dir'] || '.simcore-release/post-publish';
  ...
  const result = {
    ...
    disposition:already ? 'ADMIN_STATE_ALREADY_SYNCED' : 'POST_PUBLISH_PAYLOAD_READY',
