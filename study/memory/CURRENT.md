...
- `화면 녹화 중 2026-09-10 155706.mp4`에서 P/S/R/T 조합의 끝 키프레임 간격을 넓힌 뒤 실제 재생이 더 긴 시간에 걸쳐 진행되는 것을 확인했다. **조합 모션에서도 `키프레임 간격 = 속도` 개념 재확인 완료**로 본다.
- 해당 영상에서 사각형이 화면을 크게 가로지르며 커지고 회전하는 과장된 모습은 현재 조합의 큰 Position / Scale / Rotation 최종값이 함께 보간된 결과다.
- Composition Duration은 30초로 확장 완료했다.
- 최신 검토 `화면 녹화 중 2026-09-10 163504.mp4`에서 24초 Opacity `100%` 키프레임 생성은 확인했다.
- 그러나 24초 Position / Scale / Rotation은 약 `1577,542 / 150% / 90°`로 남아 있어 **24초 전체 원위치 복귀는 아직 미완료**다.
- 같은 녹화의 마지막 0초 상태도 약 `1577,542 / 150% / 90° / 0%`로 보여 **0초 시작 상태 재확인/정리도 필요**하다.
- 24초 복귀 미완료 상태의 상세 증거는 `study/logs/2026-09-10-after-effects-24s-return-review.md`, 오늘 전체 마감 요약은 `study/logs/2026-09-10-after-effects-session-wrap.md`에 기록했다.
...
- 따라서 다음 AE 세션의 첫 작업은 **새 기능이 아니라 24초 원위치 복귀 미완료 상태를 닫는 것**이다. 그 검증 후 필요하면 Opacity 깜빡임을 한 번 더 확인하고, 이후 오징어게임 레퍼런스 응용으로 넘어간다.
- 상세 기본 기록: `study/logs/2026-09-10-after-effects-basics.md`
...
- 24초 원위치 복귀를 Opacity 하나만으로 완료 처리하지 않는다. P/S/R/T 네 속성과 0초 시작 상태를 함께 확인한다.
...
```text
1. 24초에서 Position = 960,540 / Scale = 100% / Rotation = 0° / Opacity = 100%로 맞춘다.
2. 24초에 P/S/R/T 네 키프레임이 모두 존재하는지 확인한다.
3. 0초 시작 상태가 정상값인지 다시 확인하고 필요하면 정리한다.
4. 재생해서 중간 변형 후 24초에 원상복귀하는지 화면 증거로 검증한다.
5. 필요하면 키프레임 복사/붙여넣기로 Opacity 깜빡임을 한 번 만든다.
6. 이후 오징어게임 레퍼런스를 장면 단위로 분해해 기본 Transform을 응용한다.
