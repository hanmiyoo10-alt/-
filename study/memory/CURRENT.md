...
- 따라서 지금 바로 다음 교수 흐름은 **24초 부근 원위치 복귀/Opacity 처리**이며, 필요하면 그 전에 Opacity 키프레임 복사/붙여넣기로 깜빡임을 한 번 더 확인한다. 이후 오징어게임 레퍼런스 응용으로 넘어간다.
...
```text
1. 교수 예제의 24초 부근 원위치 복귀/Opacity 처리 흐름을 따라간다.
2. 필요하면 키프레임 복사/붙여넣기로 Opacity 깜빡임을 한 번 만든다.
3. 이후 오징어게임 레퍼런스를 장면 단위로 분해해 기본 Transform을 응용한다.
4. 익숙해진 뒤 Text Layer / Text Animator와 “집 가고 싶다” 과제 콘티로 전이한다.
```
