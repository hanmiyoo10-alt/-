...
Strategy: 난이도를 하·중·상으로 나누어 운영할 수 있는 공부에서는 `하`와 `중`을 기본 정복 범위로 삼는다. `상`은 기본적으로 선택이지만 다음 세 조건 중 하나를 만족하면 학습 대상으로 승격한다. (1) 실제 시험에서 반복적으로 자주 출제되어 사실상 필수점수 구간이 된 경우, (2) 상 난이도 개념을 알아야 중 난이도 문제를 안정적으로 풀 수 있는 경우, (3) 목표 점수가 높아 하·중 정복만으로는 부족한 경우. 하 난이도는 실수나 암기 누수 없이 거의 자동으로 맞히는 수준을 목표로 하고, 중 난이도는 개념 비교·적용·추적을 스스로 재현해 안정적으로 맞히는 수준까지 학습한다.
State: ACTIVE
Started: 2026-09-09
Last reviewed: 2026-09-09
Rationale: 사용자가 정보처리기사뿐 아니라 향후 다른 공부에서도 `하·중 전부 맞히기 / 상 선택` 원칙을 공통적으로 적용할 계획이며, 수능 경험을 통해 상 난이도 집착보다 하·중 안정화가 총점에 유리하다는 운영 원칙을 얻었다고 명시함. 이후 상 난이도의 조건부 승격 기준에도 동의함.
Evidence refs: direct user statements, 2026-09-09 strategy discussion
Success criteria: 과목이 달라도 하·중 범위를 우선 안정화해 기본점수와 실전 정확도를 확보하고, 상 난이도 투자 때문에 하·중 정답률이 흔들리지 않는다. 동시에 위 세 조건에 해당하는 상 문제를 놓치지 않고 필요한 만큼만 학습한다.
Failure / review trigger: 특정 시험에서 상 난이도 비중이 지나치게 높거나, 하·중만으로 목표 점수에 구조적으로 도달할 수 없는 경우 시험별 목표선을 조정한다.
Supersedes: NONE
Superseded by: NONE
Notes: `상 선택`은 절대적인 제외 규칙이 아니라 기본 우선순위 규칙이다. 자주 나옴 / 중을 받침 / 목표 점수상 필요 중 하나에 해당하면 상이라도 공부한다. 핵심은 난이도 이름 자체보다 점수 기여도와 학습 효율이다.
```
