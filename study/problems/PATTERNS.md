...
Last observed: 2026-09-16, AI-MATH-W3-R5
Evidence problem IDs: AI-MATH-W3-3.2, AI-MATH-W3-3.3, AI-MATH-W3-3.4, AI-MATH-W3-3.5, AI-MATH-W3-R3, AI-MATH-W3-R5
...
Success evidence: AI-MATH-W3-3.4에서는 명시적 cue 이후 `tan -> sec^2`를 정확히 적용했고, AI-MATH-W3-3.5도 기본 도함수 공식이 제공된 상태에서 정확히 적용했다. 두 사례는 cue-assisted이다. AI-MATH-W3-R3에서는 공식 힌트 없이 `(cos y)'=-sin(y)y'`를 정확히 적용했다. AI-MATH-W3-R5에서는 기본 도함수만 묻는 무힌트 문제에서 `(tan x)'=sec^2 x`와 `(sec x)'=sec x tan x`를 둘 다 정확히 회수했다.
Review trigger: 같은 날 즉시 반복보다 간격을 둔 뒤 `tan` 또는 `sec`가 연쇄법칙·곱의 미분 안에 다시 들어간 문제에서 별도 힌트 없이 정확히 회수되는지 확인한다.
Related strategy IDs: none
Notes: cue-assisted 성공만 있던 시점에는 `CANDIDATE`로 보수적으로 유지했으나, R5에서 직접 실패 대상인 `tan`과 `sec` 도함수를 둘 다 무힌트로 정확히 회수해 개선 증거가 생겼다. 다만 직접 무힌트 성공은 아직 한 번이므로 `STABLE`로 올리지 않고 `IMPROVING`을 유지한다.
