...
from benchmarks.run_o4e_scout_cell import REQUEST_TIMEOUT_SECONDS
...
class O4EMatrixError(ValueError):
    pass
...
def _read_json(path: Path | str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise O4EMatrixError(f"JSON object required: {path}")
    return value
...
def _validate_frozen_inputs(case: dict[str, Any], evidence: dict[str, Any], prompt: str) -> None:
    if case["case_id"] != EXPECTED_CASE_ID:
        raise O4EMatrixError("O4-E retrospective case identity drifted")
    if case["fixture_sha256"] != EXPECTED_FIXTURE_SHA256:
        raise O4EMatrixError("O4-E fixture digest drifted")
    if case["evidence_sha256"] != EXPECTED_EVIDENCE_SHA256:
        raise O4EMatrixError("O4-E evidence digest drifted")
    if prompt_sha256(prompt) != EXPECTED_PROMPT_SHA256:
        raise O4EMatrixError("O4-E prompt bytes drifted")
    if evidence.get("scope") != "plugin:usage-dashboard":
        raise O4EMatrixError("O4-E scope drifted")
...
def build_matrix_manifest(target_repository_sha: str) -> dict[str, Any]:
    if len(target_repository_sha) != 40 or any(ch not in "0123456789abcdef" for ch in target_repository_sha):
        raise O4EMatrixError("target_repository_sha must be lowercase 40-hex")
    ...
    manifest: dict[str, Any] = {
        ...
        "predecessor_o4d_summary_sha256": "24474cca92045afc8590dd828927b0ab62f7db657b4671603d9d3aa496960171",
    ...
    if forbidden.intersection(manifest):
        raise O4EMatrixError("winner/assignment semantics forbidden in O4-E manifest")
...
def aggregate_outputs(
    ...
    if prompt_digests != {matrix["prompt_sha256"]}:
        raise O4EMatrixError("O4-E cells did not use frozen prompt bytes")
    if schema_digests != {matrix["response_schema_sha256"]}:
        raise O4EMatrixError("O4-E cells did not use frozen response schema")
    ...
    if forbidden.intersection(summary):
        raise O4EMatrixError("winner/assignment semantics forbidden in O4-E summary")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Prepare or aggregate O4-E Scout schema-hardening validation.")
    ...
    try:
        if args.command == "prepare":
            ...
            print("O4E_MATRIX_SHA256:" + manifest["matrix_sha256"])
            print("O4E_RESPONSE_SCHEMA_SHA256:" + manifest["response_schema_sha256"])
            return 0
        summary = aggregate_outputs(args.output_root, args.matrix)
        _write_json(args.output, summary)
        print("O4E_SUMMARY_SHA256:" + summary["summary_sha256"])
        print("O4E_HARDENING_VERDICT:" + summary["hardening_verdict"])
        return 0
