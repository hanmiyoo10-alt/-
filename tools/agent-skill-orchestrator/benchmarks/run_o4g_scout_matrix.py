...
from benchmarks.run_o4g_scout_cell import MEASUREMENT_ID, O4G_MODEL_PROFILE_IDS
...
class O4GMatrixError(ValueError):
    pass
...
def _read_json(path: Path | str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise O4GMatrixError(f"JSON object required: {path}")
    return value
...
def _git(repo_root: Path, *args: str) -> str:
    ...
    if proc.returncode != 0:
        ...
        raise O4GMatrixError(f"git {' '.join(args)} failed: {detail}")
    return proc.stdout
...
def verify_frozen_source_bytes(repo_root: Path | str = ".") -> None:
    ...
    if source_shas != {EXPECTED_SOURCE_REPOSITORY_SHA}:
        raise O4GMatrixError("O4-G reused EvidencePackage contains unexpected source repository SHA")
    canonical = _git(root, "rev-parse", f"{EXPECTED_SOURCE_REPOSITORY_SHA}^{{commit}}").strip()
    if canonical != EXPECTED_SOURCE_REPOSITORY_SHA:
        raise O4GMatrixError("O4-G frozen source commit is unavailable or drifted")
    for item in evidence["sources"]:
        ...
        if start < 1 or end < start or end > len(lines):
            raise O4GMatrixError(f"O4-G frozen line range invalid for {item['path']}")
        observed = "\n".join(lines[start - 1:end])
        if observed != item["content"]:
            raise O4GMatrixError(
                f"O4-G frozen source bytes drifted for {item['source_ref']['ref']} {item['path']}"
            )
...
def build_matrix_manifest(target_repository_sha: str) -> dict[str, Any]:
    if len(target_repository_sha) != 40 or any(
        ...
        raise O4GMatrixError("target_repository_sha must be lowercase 40-hex")
    ...
    profiles = [o4f_model_profile(profile_id) for profile_id in O4G_MODEL_PROFILE_IDS]
    families = {profile["family"] for profile in profiles}
    if len(families) != 2:
        raise O4GMatrixError("O4-G requires exactly two distinct frozen model families")
    ...
    manifest: dict[str, Any] = {
        ...
        "assignment_candidate_only": False,
        "independent_assignment_case": False,
        "paired_assignment_eligible": False,
        ...
        "model_profile_ids": list(O4G_MODEL_PROFILE_IDS),
        "models": [
            {
    ...
    if forbidden.intersection(manifest):
        raise O4GMatrixError("winner/assignment semantics forbidden in O4-G manifest")
...
def aggregate_outputs(
    ...
    if not isinstance(expected_matrix_sha, str):
        raise O4GMatrixError("matrix_sha256 missing")
    matrix_base = deepcopy(matrix)
    matrix_base.pop("matrix_sha256", None)
    if canonical_sha256(matrix_base) != expected_matrix_sha:
        raise O4GMatrixError("matrix digest mismatch")
    if matrix.get("matrix_id") != MATRIX_ID:
        raise O4GMatrixError("matrix id mismatch")
    if matrix.get("diagnostic_replay_only") is not True:
        raise O4GMatrixError("O4-G matrix must remain diagnostic replay only")
    if matrix.get("assignment_candidate_only") is not False:
        raise O4GMatrixError("O4-G matrix must not become an assignment candidate")
    if matrix.get("independent_assignment_case") is not False:
        raise O4GMatrixError("O4-G matrix must not count as an independent assignment case")
    if matrix.get("paired_assignment_eligible") is not False:
        raise O4GMatrixError("O4-G matrix paired assignment eligibility must remain false")
    if matrix.get("source_repository_sha") != EXPECTED_SOURCE_REPOSITORY_SHA:
        raise O4GMatrixError("matrix frozen source SHA drifted")

    root = Path(output_root)
    rows: list[dict[str, Any]] = []
    prompt_digests: set[str] = set()
    schema_digests: set[str] = set()
    terminal_valid: list[bool] = []
    for profile_id in O4G_MODEL_PROFILE_IDS:
        ...
        if score != recomputed:
            raise O4GMatrixError(f"score read-back mismatch for {profile_id}")
        metadata = _read_json(cell_dir / "cell-metadata.json")
        if result["model"]["profile_id"] != profile_id:
            raise O4GMatrixError("cell result model identity mismatch")
        if result["model_call_count"] != 1 or result["hosted_ai_call_count"] != 0:
            raise O4GMatrixError("cell call accounting drifted")
        if metadata.get("measurement_id") != MATRIX_ID:
            raise O4GMatrixError("cell measurement identity drifted")
        if metadata.get("diagnostic_replay_only") is not True:
            raise O4GMatrixError("O4-G cell must remain diagnostic replay only")
        if metadata.get("independent_assignment_case") is not False:
            raise O4GMatrixError("O4-G cell cannot count as an independent assignment case")
        if metadata.get("assignment_eligible") is not False:
            raise O4GMatrixError("O4-G cell assignment eligibility must remain false")
        if metadata.get("assignment_basis") != "DIAGNOSTIC_ONLY":
            raise O4GMatrixError("O4-G cell assignment basis drifted")
        if metadata.get("request_timeout_seconds") != int(REQUEST_TIMEOUT_SECONDS):
            raise O4GMatrixError("cell timeout drifted")
        if metadata.get("response_schema_sha256") != matrix["response_schema_sha256"]:
            raise O4GMatrixError("cell response schema digest drifted")
    ...
    if prompt_digests != {matrix["prompt_sha256"]}:
        raise O4GMatrixError("O4-G cells did not use frozen prompt bytes")
    if schema_digests != {matrix["response_schema_sha256"]}:
        raise O4GMatrixError("O4-G cells did not use frozen strict response schema")

    both_contract_valid = all(terminal_valid) and len(rows) == 2
    observation = (
        "STRICT_UNIQUENESS_GAP_OBSERVED_CLOSED"
        if both_contract_valid
        else "STRICT_UNIQUENESS_GAP_NOT_CLOSED_OR_INCONCLUSIVE"
    )
    summary: dict[str, Any] = {
        ...
        "independent_assignment_case": False,
        "paired_assignment_eligible": False,
        "paired_contract_valid": both_contract_valid,
    ...
    if forbidden.intersection(summary):
        raise O4GMatrixError("winner/assignment semantics forbidden in O4-G summary")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Prepare, verify, or aggregate the O4-G Termux Scout uniqueness diagnostic."
    )
    ...
    try:
        if args.command == "verify-source":
            ...
            print("O4G_FROZEN_SOURCE_BYTES:VERIFIED")
            return 0
        if args.command == "prepare":
            manifest = prepare_matrix(args.target_sha, args.output)
            print("O4G_MATRIX_SHA256:" + manifest["matrix_sha256"])
            print("O4G_RESPONSE_SCHEMA_SHA256:" + manifest["response_schema_sha256"])
            print("O4G_DIAGNOSTIC_REPLAY_ONLY:true")
            return 0
