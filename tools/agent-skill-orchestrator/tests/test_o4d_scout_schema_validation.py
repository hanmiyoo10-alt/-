...
from benchmarks.resolve_o4d_request import MATRIX_ID, O4DRequestError, resolve_push
from benchmarks.run_o4d_scout_cell import REQUEST_TIMEOUT_SECONDS, main as cell_main
from benchmarks.run_o4d_scout_schema_validation import (
...
class O4DScoutSchemaValidationTests(unittest.TestCase):
    ...
    def _repo(self) -> tuple[tempfile.TemporaryDirectory[str], Path, str]:
        ...
        root = Path(temp.name)
        self._git(root, "init")
        self._git(root, "config", "user.email", "o4d@example.invalid")
        self._git(root, "config", "user.name", "O4D Test")
    ...
    def _request_commit(self, root: Path, target_sha: str, *, extra: bool = False) -> str:
        request_dir = root / ".agent-skill-o4d-requests"
        ...
        self._git(root, "add", ".agent-skill-o4d-requests/request.json")
        if extra:
    ...
    def test_request_resolver_accepts_exact_single_file_parent_bound_commit(self):
        ...
        resolved = resolve_push(
            root,
            "refs/heads/agent-skill-o4d-request/test-1",
            request_sha,
        )
        ...
        self.assertEqual(resolved["request_path"], ".agent-skill-o4d-requests/request.json")

    def test_request_resolver_fails_closed_for_wrong_branch_extra_path_and_target_mismatch(self):
        temp, root, parent = self._repo()
        self.addCleanup(temp.cleanup)
        request_sha = self._request_commit(root, parent)
        with self.assertRaises(O4DRequestError):
        ...
        with self.assertRaises(O4DRequestError):
            resolve_push(root2, "refs/heads/agent-skill-o4d-request/test-2", request_sha2)

        temp3, root3, parent3 = self._repo()
        self.addCleanup(temp3.cleanup)
        request_sha3 = self._request_commit(root3, "0" * 40)
        with self.assertRaises(O4DRequestError):
            resolve_push(root3, "refs/heads/agent-skill-o4d-request/test-3", request_sha3)
    ...
    def test_request_resolver_rejects_modified_existing_request(self):
        ...
        path = root / ".agent-skill-o4d-requests" / "request.json"
        path.write_text(
            json.dumps(
                {
                    "schema_version": 1,
        ...
        with self.assertRaises(O4DRequestError):
            resolve_push(root, "refs/heads/agent-skill-o4d-request/test-4", second)
    ...
    def test_manifest_binds_hardened_schema_and_frozen_o4c_inputs(self):
        ...
        self.assertEqual(manifest["response_schema_sha256"], canonical_sha256(scout_response_schema()))
            ...
            json.dumps(
                {
                    ...
                    "response_schema_sha256": canonical_sha256(scout_response_schema()),
    ...
    def test_cell_main_returns_success_for_canonical_invalid_terminal_evidence(self):
        ...
        with tempfile.TemporaryDirectory() as temp, patch(
            "benchmarks.run_o4d_scout_cell.execute_cell",
            return_value=(
                fake_result,
                fake_score,
                "prompt",
    ...
    def test_workflow_is_request_only_zero_credit_and_ordinary_ci_covers_it(self):
        workflow_path = REPO_ROOT / ".github" / "workflows" / "agent-skill-orchestrator-o4d-scout-schema-validation.yml"
        workflow = workflow_path.read_text(encoding="utf-8")
        self.assertIn("agent-skill-o4d-request/**", workflow)
        self.assertIn(".agent-skill-o4d-requests/*.json", workflow)
        self.assertIn("permissions:\n  contents: read", workflow)
        self.assertNotIn("workflow_dispatch", workflow)
        self.assertNotIn("copilot-requests", workflow)
        self.assertIn("run_o4d_scout_cell", workflow)
        self.assertIn("HARDENING_VALIDATED", workflow)

        ci = (REPO_ROOT / ".github" / "workflows" / "agent-skills-ci.yml").read_text(encoding="utf-8")
        path = ".github/workflows/agent-skill-orchestrator-o4d-scout-schema-validation.yml"
