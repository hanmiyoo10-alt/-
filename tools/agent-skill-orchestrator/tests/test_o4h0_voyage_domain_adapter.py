import copy
import sys
import unittest
from pathlib import Path

PACKAGE = Path(__file__).resolve().parents[1]
if str(PACKAGE) not in sys.path:
    sys.path.insert(0, str(PACKAGE))

from authority import resolve_authority
from evidence import EvidenceError, build_evidence_package
from registry import load_domain_registry
from router import RoutingError, route_task

TARGET_SHA = "3908f71122f267375ee5eccb3fa3ca85564c634e"
OTHER_SHA = "a" * 40


class O4H0VoyageDomainAdapterTests(unittest.TestCase):
    def setUp(self):
        self.registry = load_domain_registry()
        self.voyage = next(
            item for item in self.registry["domains"]
            if item["scope"] == "plugin:voyage-token-check"
        )

    def plan(self, *, task_kind="impact_analysis", registry=None):
        return route_task(
            {
                "schema_version": 1,
                "task_id": "o4h0-voyage-domain-adapter",
                "scope": "plugin:voyage-token-check",
                "task_kind": task_kind,
                "intent": "Build frozen retrospective Voyage evidence.",
                "mutation_requested": False,
                "device_truth_requested": False,
            },
            domain_registry_data=self.registry if registry is None else registry,
        )

    def observations(self):
        return [
            {
                "kind": "declared_by",
                "value": "docs/REPO_PROJECT_CATALOG.md",
                "status": "OBSERVED",
                "source_sha": TARGET_SHA,
            },
            {
                "kind": "evidence",
                "value": "voyage-token-check/DESIGN_STATUS.md",
                "status": "OBSERVED",
                "source_sha": TARGET_SHA,
            },
        ]

    def snapshot(self, observations=None):
        return resolve_authority(
            "plugin:voyage-token-check",
            TARGET_SHA,
            self.observations() if observations is None else observations,
            domain_registry_data=self.registry,
        )

    def source_blocks(self):
        return [
            {
                "path": "docs/REPO_PROJECT_CATALOG.md",
                "source_sha": TARGET_SHA,
                "start_line": 11,
                "content": "| plugin:voyage-token-check | Voyage Token Check | design-evidence-validation | voyage-token-check/** | evidence=voyage-token-check/DESIGN_STATUS.md | docs/VOYAGE_TOKEN_CHECK_GUIDELINES.md |",
            },
            {
                "path": "docs/VOYAGE_TOKEN_CHECK_GUIDELINES.md",
                "source_sha": TARGET_SHA,
                "start_line": 5,
                "content": "- Canonical project root: `voyage-token-check/`\n- Current authority class: design/evidence",
            },
            {
                "path": "voyage-token-check/DESIGN_STATUS.md",
                "source_sha": TARGET_SHA,
                "start_line": 1,
                "content": "# Voyage Token Check Design Status\nProduction implementation remains unproven.",
            },
            {
                "path": "voyage-token-check/LIVE_REFRESH_CONTRACT.md",
                "source_sha": TARGET_SHA,
                "start_line": 1,
                "content": "# Live Refresh Contract\nRefresh only while the relevant plugin view is visible.",
            },
        ]

    def test_voyage_domain_metadata_is_exact_and_has_no_release_authority(self):
        self.assertEqual(self.voyage["domain_id"], "voyage-token-check")
        self.assertEqual(self.voyage["name"], "Voyage Token Check")
        self.assertEqual(self.voyage["lifecycle"], "design-evidence-validation")
        self.assertEqual(self.voyage["primary_path"], "voyage-token-check/**")
        self.assertEqual(
            self.voyage["guidelines_path"],
            "docs/VOYAGE_TOKEN_CHECK_GUIDELINES.md",
        )
        self.assertEqual(
            {(item["kind"], item["value"]) for item in self.voyage["authority_refs"]},
            {
                ("declared_by", "docs/REPO_PROJECT_CATALOG.md"),
                ("evidence", "voyage-token-check/DESIGN_STATUS.md"),
            },
        )
        self.assertFalse(
            {item["kind"] for item in self.voyage["authority_refs"]}
            & {"release_branch", "manifest", "artifact", "release_spec_dir"}
        )
        self.assertEqual(
            self.voyage["registration_semantics"],
            "domain_metadata_only_no_skill_promotion",
        )

    def test_voyage_impact_route_and_authority_snapshot_resolve_without_release_claim(self):
        plan = self.plan()
        self.assertEqual(plan["scope"], "plugin:voyage-token-check")
        self.assertEqual(plan["execution_class"], "standard")
        snapshot = self.snapshot()
        self.assertEqual(snapshot["overall_status"], "RESOLVED")
        self.assertEqual(snapshot["blockers"], [])
        self.assertEqual(
            [(item["kind"], item["status"]) for item in snapshot["authorities"]],
            [("declared_by", "OBSERVED"), ("evidence", "OBSERVED")],
        )

    def test_release_lookup_fails_closed_without_inventing_voyage_release_authority(self):
        with self.assertRaisesRegex(RoutingError, "release_lookup requires registered release_branch"):
            self.plan(task_kind="release_lookup")

    def test_canonical_builder_emits_exact_voyage_authority_classes(self):
        package = build_evidence_package(
            self.plan(),
            self.snapshot(),
            self.source_blocks(),
            domain_registry_data=self.registry,
        )
        classes = {item["path"]: item["authority_class"] for item in package["sources"]}
        self.assertEqual(classes["docs/REPO_PROJECT_CATALOG.md"], "declared_by")
        self.assertEqual(classes["docs/VOYAGE_TOKEN_CHECK_GUIDELINES.md"], "guidelines")
        self.assertEqual(classes["voyage-token-check/DESIGN_STATUS.md"], "evidence")
        self.assertEqual(classes["voyage-token-check/LIVE_REFRESH_CONTRACT.md"], "domain_primary")
        self.assertEqual(package["scope"], "plugin:voyage-token-check")
        self.assertEqual(package["target_repository_sha"], TARGET_SHA)

    def test_unresolved_explicit_voyage_evidence_authority_fails_closed(self):
        snapshot = self.snapshot([self.observations()[0]])
        with self.assertRaisesRegex(EvidenceError, "authority is not OBSERVED"):
            build_evidence_package(
                self.plan(),
                snapshot,
                [self.source_blocks()[2]],
                domain_registry_data=self.registry,
            )

    def test_exact_authority_sha_mismatch_and_out_of_domain_path_fail_closed(self):
        bad = copy.deepcopy(self.source_blocks()[0])
        bad["source_sha"] = OTHER_SHA
        with self.assertRaisesRegex(EvidenceError, "source_sha mismatch"):
            build_evidence_package(
                self.plan(), self.snapshot(), [bad], domain_registry_data=self.registry
            )
        with self.assertRaisesRegex(EvidenceError, "outside registered domain evidence boundaries"):
            build_evidence_package(
                self.plan(),
                self.snapshot(),
                [{
                    "path": "plugins/devpass/latest.js",
                    "source_sha": TARGET_SHA,
                    "start_line": 1,
                    "content": "x",
                }],
                domain_registry_data=self.registry,
            )


if __name__ == "__main__":
    unittest.main()
