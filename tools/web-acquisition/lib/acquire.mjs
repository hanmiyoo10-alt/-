...
async function installNetworkGuard(context, page, options) {
  ...
  if (options.revealMode) {
    await page.addInitScript(() => {
      window.__u26RevealArmed = false;
      window.__u26RevealDownloadBlocked = null;
      const recordDownload = (anchor) => {
        if (!window.__u26RevealArmed) return false;
        window.__u26RevealDownloadBlocked = {
          attempted: true,
      ...
      Object.defineProperty(HTMLElement.prototype, 'click', {
        configurable: true,
        writable: true,
        value: function guardedElementClick(...args) {
      ...
      document.addEventListener('click', (event) => {
        if (!window.__u26RevealArmed) return;
    ...
    cdp.on('Page.downloadWillBegin', (event) => {
      if (revealArmed) recordEffect('REVEAL_DOWNLOAD_FORBIDDEN', { url: event.url });
    });
    const frameTree = await cdp.send('Page.getFrameTree').catch(() => null);
    mainFrameId = frameTree?.frameTree?.frame?.id ?? null;
  }

  const effectWaiters = new Set();
  function recordEffect(reason, event = {}) {
    if (blockedEffects.length < 20) {
      blockedEffects.push(boundedEffectRecord(reason, event));
    }
    for (const wake of effectWaiters) wake();
    effectWaiters.clear();
  }

  async function waitForBlockedEffect(timeoutMs = 0) {
    if (blockedEffects.length > 0) return true;
    if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) return false;
    return new Promise((resolve) => {
      let timer = null;
      const wake = () => {
        if (timer) clearTimeout(timer);
        effectWaiters.delete(wake);
        resolve(blockedEffects.length > 0);
      };
      timer = setTimeout(() => {
        effectWaiters.delete(wake);
        resolve(false);
      }, timeoutMs);
      effectWaiters.add(wake);
    });
  }
